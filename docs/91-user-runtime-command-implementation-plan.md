...
```bash
docker run --rm -v "$HOME/.local:/install" ghcr.io/wamukat/a2o-engine:latest a2o host install --output-dir /install/bin --share-dir /install/share/a2o --runtime-image ghcr.io/wamukat/a2o-engine:latest
...
run-once / loop は host launcher 内部 flow として保持するが、通常利用者向け command surface には出さない。

Portal workspace-local の Taskfile runtime namespace は削除済みであり、A2O release の利用者に作らせるものではない。

`--package` は `a2o project bootstrap` の入力であり、単一 project package の path を指す。bootstrap はカレント workspace に runtime instance config を作成する。bootstrap 後の `a2o kanban ...` と `a2o agent install` は、カレントディレクトリから上方向に instance config を探索して対象 package を解決するため、通常操作では package path を毎回指定しない。
...
- Host launcher `a2o`
  ...
  - internal run-once / loop flow を保持する。ただし通常利用者向け public surface には `run-once` / `loop` を出さず、`a2o project bootstrap`、`a2o kanban ...`、`a2o agent install` を正規入口にする。
...
- Runtime instance config
  ...
  - `a2o kanban ...` / `a2o agent install` はこの config を探索して読む。
...
- `[done]` GHCR の正式 image ref を `ghcr.io/wamukat/a2o-engine:latest` として固定し、quickstart の placeholder を置換する。
...
- `[done]` `a2o kanban up` / `doctor` と `a2o kanban ...` は bootstrap 済み runtime instance config を読む。
- `[done]` `a2o agent install` は bootstrap 済み runtime instance config から compose file / compose project / runtime service を読む。
- `[done]` compose file / project name / ports / storage dir
- `[done]` kanban service URL / provider internal URL
- `[done]` manifest path
- `[done]` live ref
- `[done]` repo source path
- `[done]` agent workspace root
- `[done]` required bins
- `[done]` worker command / worker args

loader は fail-fast とし、未設定時に暗黙 fallback で Portal 固有値を補わない。

### Slice 4: generic runtime run-once

host launcher `internal run-once flow` が次を実行する。

- `[done]` host launcher 内部 helper として `internal run-once flow` を追加する。通常利用者向け command としては公開しない。
...
`scripts/a3-projects/portal/runtime/run_once.sh` は削除済み。root Taskfile の互換入口は、workspace root を Go module と誤認しないよう `the internal run-once helper` を呼ぶ。

### Slice 5: runtime loop

`internal loop flow` を追加する。
...
Portal root Taskfile 互換入口が残る場合でも、呼び先は host launcher 内部 helper に限定する。通常利用者向けの現行入口は `a2o project bootstrap` / `a2o kanban ...` / `a2o agent install` であり、Portal Taskfile から internal flow を直接案内しない。
...
- `[done]` `internal loop flow` の host launcher command を実装し、有限 cycle validation を可能にする。
- `[done]` GHCR の正式 image ref を `ghcr.io/wamukat/a2o-engine:latest` として固定し、`90-user-quickstart.md` と本計画の placeholder を置換する。
- `[done]` GitHub Actions workflow `publish-a2o-engine.yml` を追加し、`main` push / manual dispatch で `ghcr.io/wamukat/a2o-engine:latest` と `sha-*` tag を publish する。PR では push せず Docker build のみ確認する。
- `[done]` GitHub Actions の JavaScript action runtime 移行に合わせ、publish workflow の外部 actions は Node.js 24 対応 major (`actions/checkout@v5`, Docker actions v4/v6/v7) に更新済み。各 action の `action.yml` で `runs.using: node24` を確認した。
- `[done]` local release image equivalent (`a3-portal-bundle-a3-runtime`) で `a2o host install` -> `a2o project bootstrap` -> `a2o kanban up` -> `a2o kanban doctor` -> `a2o agent install` -> `internal command-plan` の smoke を実行する。public launcher `a2o`、compat alias `a3`、public share dir `share/a2o`、public agent path `.work/a2o-agent/bin/a2o-agent` を確認済み。
...
- Portal workspace-local の runtime Taskfile entrypoint は削除済みであり、長い実行ロジックを保持しない。
