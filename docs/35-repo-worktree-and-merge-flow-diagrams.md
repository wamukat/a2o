# A3-v2 Repo / Worktree / Merge Model

対象読者: A3-v2 設計者 / 実装者 / 運用者
文書種別: 設計メモ

この文書は、A3-v2 が `workspace root`、product repo、task workspace をどう使い分けるかと、merge phase をどのモデルで扱うかをまとめて説明する。
...
flowchart TB
  subgraph ROOT["workspace root: mypage-prototype/"]
    ...
    A3V2["a3-v2/<br/>scheduler / sync / phase orchestration"]
    STORAGE[".work/a3-v2/portal-kanban-scheduler-auto/<br/>tasks.json / runs.json / workspaces / quarantine"]
  ...
  KANBAN -->|task snapshot / labels / relations| A3V2
  A3CFG --> A3V2
  A3V2 <--> STORAGE
...
- `tasks.json` / `runs.json` は A3-v2 の運転用 state
- `member-portal-ui-app` / `member-portal-starters` が source repo
- `.work/a3-v2/.../workspaces/...` は task 実行用 worktree

## 3. Phase ごとの動き

```mermaid
flowchart LR
  K["Kanban task"] --> S["A3-v2 sync<br/>Kanban is master"]
  S --> P["phase plan"]
...
- conflict / checkout failure / dirty tree は `blocked`
...
そのため、A3-v2 が root repo を自動同期するのは次の条件を満たす場合に限る。
...
- A3-v2 が cache / execution surface として持つもの
