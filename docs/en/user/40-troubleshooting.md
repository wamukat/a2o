...
If `describe-task` reports `Task not found` for a Kanban task that is still in an active lane, upgrade to A2O 0.5.80 or later, restart the runtime container with the matching image, and run `a2o runtime run-once` or resume the scheduler. A2O 0.5.80 can recover active `In progress`, `In review`, `Inspection`, and `Merging` tasks from Kanban even when the runtime task record is missing.
