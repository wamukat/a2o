...
When Kanbalone supports non-blocking relations, A2O also creates a `related` relation from the requirement source ticket to the generated implementation parent. The relation is traceability only; it does not make the source ticket runnable and it does not replace child `subtask` relations or dependency `blocked` relations.
