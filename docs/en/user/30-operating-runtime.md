...
When a successful implementation worker returns `operator_proposals`, `describe-task` prints `operator_proposals_count=`, one `operator_proposal` summary line per proposal, and title/summary/suggested-action detail lines. Each proposal also has a logical evidence path under the latest phase record. Normal `watch-summary` output does not include proposal details. A2O appends a compact Markdown proposal section to the successful implementation completion comment, showing at most three proposals; use `describe-task` for the full preserved result.
...
When the source ticket was imported from an external issue, A2O keeps the source ticket as the requirement artifact and does not re-import the external issue for the generated parent. A2O records normalized remote metadata in child-creation evidence as `source_remote` and, when Kanbalone v0.9.28 or newer is available, writes a non-tracking generated parent `externalReferences[source]` entry. Older external Kanbalone endpoints continue with relation/evidence traceability and record a child-creation warning instead of copying remote metadata into ticket text. Generated parent and draft child bodies do not receive copied remote metadata; workers should follow the relation, the generated parent's external reference, or evidence when they need provenance.

Each completed stage leaves a short comment on the source ticket so operators can follow progress from Kanban. `a2o runtime watch-summary` also shows `trigger:investigate` source tickets in its `Decomposition` section; before any evidence is written they appear as `state=queued`, and while decomposition is active the section shows the current stage, such as `stage=investigate`, `stage=propose`, `stage=review`, or `stage=create_children`. Active decomposition sources also make the scheduler summary show `Scheduler: running` and the task tree marks the source with the running marker. Detailed evidence is stored under the runtime storage directory in `decomposition-evidence/<task>/`; `a2o runtime decomposition status <task-ref>` shows the current decomposition evidence summary, and `a2o runtime describe-task <task-ref>` gives the broader task state.
