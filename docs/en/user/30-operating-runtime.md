...
When the source ticket was imported from an external issue, A2O keeps the source ticket as the requirement artifact and does not re-import the external issue for the generated parent. Instead, A2O copies normalized remote metadata into the generated parent description/comment and the child-creation evidence as `source_remote`, while preserving the Kanbalone `related` relation from the requirement source to the generated implementation parent. Draft children do not receive direct remote metadata; they stay local implementation planning tickets under the generated parent.
