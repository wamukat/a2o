...
When the source ticket was imported from an external issue, A2O keeps the source ticket as the requirement artifact and does not re-import the external issue for the generated parent. A2O records normalized remote metadata only in child-creation evidence as `source_remote`, while preserving the Kanbalone `related` relation from the requirement source to the generated implementation parent. Generated parent and draft child bodies do not receive copied remote metadata; workers should follow the relation or evidence when they need provenance.
