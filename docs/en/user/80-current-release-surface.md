# A2O 0.5.71 Current Release Surface

This document describes the currently supported A2O 0.5.71 user surface and validation boundary.
...
- Implementation completion hooks: `runtime.phases.implementation.completion_hooks.commands` runs project-owned commands after implementation worker success and before review/verification. Hooks can be `mutating` or `check`; failures produce implementation rework feedback instead of advancing the task. See [90-project-package-schema.md#implementation-completion-hooks](90-project-package-schema.md#implementation-completion-hooks).
- Agent-materialized execution loads implementation completion hooks from `project.yaml`; explicit `--agent-implementation-completion-hook` CLI options remain an override for runtime-generated command lines.
...
- Kanbalone adapter and bootstrap tooling, defaulting to Kanbalone `v0.9.34`
- Bundled Kanbalone `v0.9.34` keeps parent ticket hierarchy indicators visible even when all child tickets are hidden by active filters, including resolved-only children. Ticket detail and summary API responses include `hasChildren` so hierarchy display can use persisted relations instead of filtered board lists.
...
- GHCR runtime image tags: `latest`, `0.5.71`, and `sha-*`
...
- Decomposition command execution now depends on the host launcher and packaged host agent. Upgrade the host launcher/shared assets and runtime image together before using `a2o runtime decomposition investigate`, `propose`, or `review`; otherwise an older host launcher may not recognize the decomposition host-agent command path. A typical upgrade is:
  - install the new launcher from the release image: `docker run --rm -v "$PWD/.work/a2o:/out" ghcr.io/wamukat/a2o-engine:0.5.71 a2o host install --output-dir /out/bin --share-dir /out/share`
  - update project runtime image references to `ghcr.io/wamukat/a2o-engine:0.5.71`
  - restart the runtime container with the new image before running decomposition commands.
- Decomposition draft acceptance in 0.5.71 enables generated-parent automation by default. After upgrading, `a2o runtime decomposition accept-drafts` adds `trigger:auto-parent` and the accepted child `repo:*` label union to the generated parent unless `--no-parent-auto` is passed. Update both the host launcher/shared assets and runtime image before relying on this behavior.
