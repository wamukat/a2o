...
- If a decomposition source ticket was imported from an external issue, A2O records normalized remote metadata on the generated parent and in child-creation evidence as `source_remote`; draft children remain local planning tickets and do not receive copied remote metadata.
