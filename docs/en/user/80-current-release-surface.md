# A2O 0.5.60 Current Release Surface

This document describes the currently supported A2O 0.5.60 user surface and validation boundary.
...
- `a2o runtime logs <source-ref> --follow` follows decomposition source tickets by polling decomposition status and streaming available investigate/propose/review action logs instead of reporting decomposition follow as unsupported.
...
- GHCR runtime image tags: `latest`, `0.5.60`, and `sha-*`
...
- Decomposition command execution now depends on the host launcher and packaged host agent. Upgrade the host launcher/shared assets and runtime image together before using `a2o runtime decomposition investigate`, `propose`, or `review`; otherwise an older host launcher may not recognize the decomposition host-agent command path. A typical upgrade is:
  - install the new launcher from the release image: `docker run --rm -v "$PWD/.work/a2o:/out" ghcr.io/wamukat/a2o-engine:0.5.60 a2o host install --output-dir /out/bin --share-dir /out/share`
  - update project runtime image references to `ghcr.io/wamukat/a2o-engine:0.5.60`
  - restart the runtime container with the new image before running decomposition commands.
- Decomposition observability improvements in 0.5.60 require updating both the host launcher and runtime image. Older host launchers still print the pre-0.5.60 decomposition log fallback behavior, and older runtime images do not report the new watch-summary stage field.
