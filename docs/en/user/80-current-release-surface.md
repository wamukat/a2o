# A2O 0.5.62 Current Release Surface

This document describes the currently supported A2O 0.5.62 user surface and validation boundary.
...
- When decomposition proposal review ends with a blocked disposition, A2O publishes the source ticket as blocked through the Kanban status publisher so the `blocked` label and lane transition are applied and the source is not reprocessed as an active investigation loop.
...
- GHCR runtime image tags: `latest`, `0.5.62`, and `sha-*`
...
- Decomposition command execution now depends on the host launcher and packaged host agent. Upgrade the host launcher/shared assets and runtime image together before using `a2o runtime decomposition investigate`, `propose`, or `review`; otherwise an older host launcher may not recognize the decomposition host-agent command path. A typical upgrade is:
  - install the new launcher from the release image: `docker run --rm -v "$PWD/.work/a2o:/out" ghcr.io/wamukat/a2o-engine:0.5.62 a2o host install --output-dir /out/bin --share-dir /out/share`
  - update project runtime image references to `ghcr.io/wamukat/a2o-engine:0.5.62`
  - restart the runtime container with the new image before running decomposition commands.
- Decomposition observability improvements introduced in 0.5.61 still require updating both the host launcher and runtime image. Older host launchers still print the pre-0.5.61 decomposition log fallback behavior, and older runtime images do not write action-specific decomposition live logs such as `decomposition_propose.log`.
