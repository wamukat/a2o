...
When a source ticket carries imported external issue metadata, the generated implementation parent is still a separate local Kanban ticket. A2O must not duplicate the external import onto that parent because Kanbalone treats an external issue import as a single source ticket. A2O records normalized `source_remote` metadata in child-creation evidence, writes a non-tracking generated parent `externalReferences[source]` entry through Kanbalone v0.9.28 or newer, keeps the source-to-generated-parent `related` relation, and does not copy remote metadata into generated parent or draft child bodies. If an older external Kanbalone endpoint lacks external references, child creation remains non-fatal and records a warning in child-creation evidence.
