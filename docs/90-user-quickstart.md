...
## End-to-end Runtime Target

A2O の利用者向け完成形では、task を kanban に置いた後、公開 command だけで implementation、verification、merge、evidence 記録まで進める。

現行 public launcher は、この execution loop command をまだ公開していない。A2O#267 が完了するまでは、次の流れは目標仕様であり、実行手順ではない。
...
4. A2O#267 で追加する A2O-owned runtime command を実行する。
...
現行 baseline では、同じ runtime flow を内部 Engine CLI で検証している。利用者向け release では、A2O#267 でこの差分を閉じるまで、A2O は end-to-end automation を一般利用者へ公開できる状態とは扱わない。
...
- Full runtime execution の baseline reproduction は、まだ内部 Engine CLI を直接使う手順を含む。通常利用者向けには、A2O#267 で A2O-owned runtime command に閉じる必要がある。
