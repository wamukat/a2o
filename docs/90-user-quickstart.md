...
`a2o doctor` は release-readiness 用の一括診断で、project package、required command、repo clean 状態、agent install、kanban volume / service、runtime image digest を確認する。問題がある check は `status=blocked` と `action=...` を出す。
