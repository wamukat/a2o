...
4. `a2o runtime run --until-idle` を実行する。
...
現行 baseline では、この E2E runtime command はまだ公開 surface として閉じていない。内部 Engine CLI で同じ runtime flow は検証済みだが、利用者向け release では A2O#267 で `a2o runtime run --until-idle` 相当の command に閉じる。
...
- Full runtime execution の baseline reproduction は、まだ内部 Engine CLI を直接使う手順を含む。通常利用者向けには、A2O#267 で `a2o runtime run --until-idle` 相当の公開コマンドに閉じる必要がある。
