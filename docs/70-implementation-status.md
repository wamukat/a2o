...
- [ ] SoloBoard bundled kanban and agent runtime packaging
  ...
  さらに `ITERATIONS=6 INTERVAL_SECONDS=30 task a3:portal:runtime:observe` で runtime doctor / watch-summary / show-state / disk usage を約 3 分間反復観測し、全 iteration で active / queued / blocked が 0 の idle 状態を確認した。legacy direct `a3:portal:runtime:run-once` は docker:a3 内で project verification を実行しうるため、diagnostic validation `Portal#43` で失敗条件を確認した後、後続整理で entrypoint 自体を削除した。Docker dev-env agent loop は historical validation として完了し、2026-04-13 以降の通常検証は host-local agent 経路に固定した。
  2026-04-12 の再確認では standalone `task soloboard:doctor/bootstrap/validation`、runtime `task a3:portal:runtime:doctor/bootstrap`、Docker dev-env agent validation 群、`task a3:portal:runtime:agent-artifact-cleanup -- --dry-run`、`task a3:portal:cleanup`、`ITERATIONS=1 INTERVAL_SECONDS=1 task a3:portal:runtime:observe` を連続実行した。これらの Docker dev-env task は 2026-04-13 の配布整理で削除済みで、証跡だけを historical evidence として保持する。
  ...
  2026-04-12 の方針更新で、A3 完成前の最終確認は `A3 Engine on Docker` + `A3 Agents on Local(~/workspace/mypage-prototype)` を標準対象にする。実 Portal source の `repo:both` parent/full verification は完成判定または release candidate 判定で実行する heavy validation とし、日常の軽量 validation には含めない。
  残課題:
  ...
  - host local `a3-agent` と Docker 上 A3 runtime の protocol validation を root task として固定し、final validation matrix の `T0` から `T5` を host-local agent 主経路の軽量 validation として通す
  - 実 Portal source の `repo:both` parent/full verification validation は完成判定または release candidate 判定で実行する。毎回の回帰 validation にはしない
