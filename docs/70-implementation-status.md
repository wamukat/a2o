...
- [ ] SoloBoard bundled kanban and agent runtime packaging
  ...
  - Docker compose bundle を `docker:a3` + `docker:soloboard` + optional `docker:dev-env(a3-agent)` の形へ更新する。workspace root の bundle smoke で `a3-runtime` control plane から `a3-agent` service へ job を流し、result / artifact upload まで確認済み
