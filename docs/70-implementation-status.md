...
- [ ] SoloBoard bundled kanban and agent runtime packaging
  ...
  残課題:
  - current A3 がまだ使っていない command surface を parity 確認する。standalone SoloBoard smoke と bundled agent smoke sweep は再確認済みで、残りは operator が通常運用で触りにくい診断/retention 系 command surface の長時間観測である
  - repeated scheduler-loop と長時間運用で read-after-write 揺れが追加 hardening を要しないか確認する。短時間の idle repeated observation、agent 正規経路の 2 周 mutation loop、2026-04-12 の 3 iteration bundle observe、bundle archive-state による active storage 退避、post-archive idle 確認、post-archive recovery smoke、bundled agent smoke sweep 後の idle observe は完了済みで、残りは blocked diagnosis evidence retention を含む長い時間窓の常駐 loop 観測である
