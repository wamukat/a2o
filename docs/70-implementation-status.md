...
- [ ] SoloBoard bundled kanban and agent runtime packaging
  ...
  さらに `ITERATIONS=6 INTERVAL_SECONDS=30 task a3:portal:runtime:observe` で runtime doctor / watch-summary / show-state / disk usage を約 3 分間反復観測し、全 iteration で active / queued / blocked が 0 の idle 状態を確認した。legacy direct `a3:portal:runtime:run-once` は docker:a3 内で project verification を実行しうるため、diagnostic canary `Portal#43` で失敗条件を確認した後、後続整理で entrypoint 自体を削除した。Docker dev-env agent loop は historical smoke として完了し、2026-04-13 以降の通常検証は host-local agent 経路に固定した。
  2026-04-12 の再確認では standalone `task soloboard:doctor/bootstrap/smoke`、runtime `task a3:portal:runtime:doctor/bootstrap`、Docker dev-env agent smoke 群、`task a3:portal:runtime:agent-artifact-cleanup -- --dry-run`、`task a3:portal:cleanup`、`ITERATIONS=1 INTERVAL_SECONDS=1 task a3:portal:runtime:observe` を連続実行した。これらの Docker dev-env task は 2026-04-13 の配布整理で削除済みで、証跡だけを historical evidence として保持する。
  ...
  - Docker compose runtime を `docker:a3` + `docker:soloboard` の形へ整理する。project command は compose 内 agent image ではなく host/project-dev-env の `a3-agent` release binary が担当する。historical Docker dev-env smoke は protocol evidence として残すが、current release surface からは削除済み
