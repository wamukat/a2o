...
- [ ] SoloBoard bundled kanban and agent runtime packaging
  ...
  - `bundle` という operator-facing task 名は実装由来で分かりにくいため、A3 Engine on Docker を標準 runtime とする前提で retired Portal runtime Taskfile namespace へ rename 済み。旧 `a3:portal:bundle:*` は短期 maintenance alias とし、現在の runbook / watch-summary では retired runtime Taskfile namespace を正規入口として扱う
