...
- [ ] SoloBoard bundled kanban and agent runtime packaging
  ...
  残課題:
  - current A3 がまだ使っていない command surface を parity 確認する。standalone SoloBoard smoke、bundled agent smoke sweep、archived blocked diagnosis の診断/cleanup/repair dry-run surface は再確認済みで、残りは本当に未使用の operator command がないかの最終棚卸しである
