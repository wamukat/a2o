...
- [ ] SoloBoard bundled kanban and agent runtime packaging
  ...
  さらに `ITERATIONS=6 INTERVAL_SECONDS=30 task a3:portal:runtime:observe` で runtime doctor / watch-summary / show-state / disk usage を約 3 分間反復観測し、全 iteration で active / queued / blocked が 0 の idle 状態を確認した。legacy direct `a3:portal:runtime:run-once` は docker:a3 内で project verification を実行しうるため、diagnostic canary `Portal#43` で失敗条件を確認したうえで明示 opt-in guard に下げた。Docker dev-env agent loop は historical smoke として完了し、2026-04-13 以降の通常検証は host-local agent 経路に固定した。
