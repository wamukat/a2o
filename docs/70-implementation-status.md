...
- [ ] SoloBoard bundled kanban and agent runtime packaging
  ...
  - workspace materialization / dirty check / cleanup の owner を agent runtime として実装し、A3 は source descriptor と workspace descriptor / evidence を検証する形に寄せる。現時点では contract-only slice として Ruby `AgentWorkspaceRequest`、`AgentJobRequest.workspace_request`、Go `WorkspaceRequest` JSON shape を追加済み。次は Go agent 側の `local_git` alias + `worktree_detached` materializer unit を実装する
