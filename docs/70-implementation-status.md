...
- [ ] SoloBoard bundled kanban and agent runtime packaging
  ...
  - `bundle` という operator-facing task 名は実装由来で分かりにくいため、A3 Engine on Docker を標準 runtime とする前提で `a3:portal:runtime:*` へ rename する。旧 `a3:portal:bundle:*` は短期 maintenance alias とし、docs / logs / watch-summary の表記からも段階的に外す
