...
- [ ] SoloBoard bundled kanban and agent runtime packaging
  ...
  - Docker compose bundle を `docker:a3` + `docker:soloboard` + optional `docker:dev-env(a3-agent)` の形へ更新する。workspace root の bundle smoke で `a3-runtime` control plane から `a3-agent` service へ job を流し、result / artifact upload まで確認済み。さらに `task a3:portal:bundle:agent-worker-gateway-smoke` で `execute-until-idle --worker-gateway agent-http` が compose `a3-agent` 経由で implementation worker result を返し、対象 SoloBoard task を `Inspection` まで進められることを確認済み。`task a3:portal:bundle:agent-verification-smoke` では Portal dev-env agent image 経由で `run-verification --verification-command-runner agent-http` を実行し、remediation と verification の 2 command が agent job として成功し、対象 SoloBoard task が `Merging` へ進むことを確認済み
  - Portal starters full verification は実 `member-portal-starters` source で `a3-agent` 経由確認済み。UI app 側を含む full verification canary は次の project-specific integration slice として残る
