...
- [ ] SoloBoard bundled kanban and agent runtime packaging
  ...
  - `a3-agent` の JobRequest / JobResult / lifecycle / policy / transport を実装可能な粒度で固定する。特に HTTP transport では log/artifact を local path 参照ではなく A3-managed artifact store へ upload/stream する。Ruby domain 側では `AgentJobRequest` / `AgentJobResult` / upload-backed artifact reference / agent workspace descriptor の最小 contract を追加済みで、JSON-backed job store、file-backed artifact store、pull handler、artifact upload endpoint、`a3 agent-server` の最小 HTTP entrypoint、Ruby reference agent (`a3-agent`) の 1 job worker loop も追加済み。Go 側は `agent-go` module として 1 job worker loop を標準 library のみで build / test できる scaffold まで追加済み。さらに worker phase と HTTP pull agent を接続する `AgentWorkerGateway` の first slice を追加し、既存 worker request/result protocol を `WorkerProtocol` に抽出した。`agent-http` gateway は first slice では explicit `same-path` shared workspace mode と既存 `a3 agent-server` を前提に、agent job 完了後も `.a3/worker-result.json` を正本として validation / `changed_files` canonicalization を行う
  - workspace materialization / dirty check / cleanup の owner を agent runtime として実装し、A3 は source descriptor と workspace descriptor / evidence を検証する形に寄せる。現時点では Ruby `AgentWorkspaceRequest`、`AgentJobRequest.workspace_request`、Go `WorkspaceRequest` JSON shape に加え、Go agent 側の `local_git` alias + `worktree_detached` materializer unit まで追加済み。materializer は agent config 相当の `WorkspaceRoot` と `SourceAliases` を受け取り、leaf worktree の prepare / cleanup と dirty source hard-fail を unit test で固定した。次は HTTP worker loop へ materializer を接続する
