...
- [ ] single / child 向け phase redesign slice
  現状: fresh な `single` / `child` は `implementation completed -> verification` へ進み、kanban status も `Inspection` へ遷移する。implementation worker は optional `review_disposition(kind=completed)` を返せるようになり、self-review clean の evidence は implementation run record と operator view に保持できる。あわせて `single` / `child` では `review` を canonical support phase から外した。watch-summary など operator 表示は canonical 4 phase を維持している。残りは external kanban / CLI surface を 3 段 canonical へ揃えること。
