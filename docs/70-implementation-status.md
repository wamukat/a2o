...
- [ ] SoloBoard bundled kanban and agent runtime packaging
  ...
  - Portal full verification は、現時点では smoke 用 fake repo ではなく実 `member-portal-starters` / `member-portal-ui-app` source で `a3-agent` 経由再実行する必要が残る
