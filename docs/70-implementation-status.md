...
- [ ] SoloBoard bundled kanban and agent runtime packaging
  ...
  2026-04-12 の再確認では standalone `task soloboard:doctor/bootstrap/smoke`、bundle `task a3:portal:bundle:doctor/bootstrap`、`task a3:portal:bundle:agent-smoke`、`task a3:portal:bundle:agent-worker-gateway-smoke` (`Portal#95`)、`task a3:portal:bundle:agent-verification-smoke` (`Portal#96`)、`task a3:portal:bundle:agent-parent-topology-smoke` (`Portal#97/#98/#99`)、`task a3:portal:bundle:agent-artifact-cleanup -- --dry-run`、`task a3:portal:cleanup`、`ITERATIONS=1 INTERVAL_SECONDS=1 task a3:portal:bundle:observe` を連続実行した。standalone smoke の `Portal#5/#6` も含め、作成された smoke task は確認後に `Done` へ閉じ、最終 state は `active_runs=0`, `queued_tasks=0`, `blocked_tasks=0` だった。
  同日の diagnostic / retention surface 確認では、archived `Portal#43` blocked run (`572a705b-3d81-4409-b370-38cc34963b18`) を scratch copy へ複製し、`show-run`, `show-blocked-diagnosis`, `doctor-runtime`, `repair-runs` dry-run, `show-state`, `cleanup-terminal-workspaces` dry-run, `quarantine-terminal-workspaces` を実行した。blocked diagnosis は `JAVA_HOME environment variable is not defined correctly` まで追跡でき、scratch は削除、archive 正本は保持した。さらに cap 指定 artifact cleanup dry-run と `ITERATIONS=4 INTERVAL_SECONDS=45 task a3:portal:bundle:observe` も成功し、全 iteration で active / queued / blocked は 0、host disk 空きは約 89GiB を維持した。
  ...
  - repeated scheduler-loop と長時間運用で read-after-write 揺れが追加 hardening を要しないか確認する。短時間の idle repeated observation、agent 正規経路の 2 周 mutation loop、2026-04-12 の 3 iteration bundle observe、bundle archive-state による active storage 退避、post-archive idle 確認、post-archive recovery smoke、bundled agent smoke sweep 後の idle observe、diagnostic surface 確認後の 4 iteration observe は完了済みで、残りは必要なら overnight 相当の常駐 loop 観測に限られる
  ...
  - `bundle` という operator-facing task 名は実装由来で分かりにくいため、最終 validation 前後で `a3:portal:docker-runtime:*` または `a3:portal:local-runtime:*` へ rename する。旧 `a3:portal:bundle:*` は短期 maintenance alias とし、docs / logs / watch-summary の表記からも段階的に外す
