...
この文書は、A2O の current public surface と、A3 内部互換として残る `runtime` / `scheduler` 用語の境界を定義する。

## 方針

- 利用者向けの入口名は `a2o host install`、`a2o project bootstrap`、`a2o kanban up` / `doctor` / `url`、`a2o agent install`、`a2o-agent` に限定する。
...
- Go release binary としての `a2o-agent`
...
完成形では A3 Engine container が runtime loop process を持ち、kanban selection / run state / agent job queue を管理する。public path では `a2o-agent` が host/project-dev-env 上で job を poll/execution する worker である。内部互換名としての `a3-agent` は scheduler を持たない。
...
- Compatibility agent binary name: `a3-agent`
- Engine image bundled agent package maintenance command: `a3 agent package list/export/verify`
