...
### executor command contract

orchestrator は `codex exec` に直接依存するのではなく、汎用的な worker-executor contract に依存するべきです。ただし v1 では provider adapter class を先に増やさず、まず `launcher.json` による command template contract を固定します。

executor command contract が持つ責務は次です。

- prompt transport
- process launch argv template
- result collection
- schema handoff
- runtime 固有 env の注入

A3 Engine は `model`、`reasoning_effort`、`--model`、`model_reasoning_effort` のような provider 固有 option を解釈しません。Codex を使う場合も、それらは project launcher config の command 配列に閉じ込めます。
...
provider adapter は、command template だけでは auth、streaming、result 回収、tool sandbox 差分を吸収できないと確認できた段階で導入します。adapter 抽象を v1 completion の前提にはしません。
...
### `2521` 着手前に決めておくべき external interface

`2533` で runner entrypoint はできたが、`2521` に入るには次の interface を先に固定する必要がある。

- executor command interface
  - one-shot 実行コマンドの argv template
...
`2521` では、この interface 定義を先に code と文書へ落とし、その後に launchd 依存の実装を scheduler / runtime 側へ移す。
...
### Step 4. executor command contract を切り出す

manifest 境界が安定したら、`Codex` 固有の launch behavior を `launcher.json` の command template に移します。
...
目的は Codex を即時置き換えることではなく、runtime 固有ロジックと provider 固有 option 解釈を automation engine から追い出すことです。provider adapter は command template では不足する事実が出た場合の後続選択肢とします。
...
- runtime 固有 command argv template
- auth / config handoff
- prompt / result transport
- schema file handoff
...
4. executor runtime 依存を command template 化する
