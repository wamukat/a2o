...
- 未実装 / 未完
  - `a3-agent` の production daemon 管理 / auth / 長時間運用 hardening。service 化の最小入口として loop mode と service-manager template rendering は実装済みで、installer から install/load/enable する段階が残る
...
- `a3-agent` の Go single binary service template を使った installer 側 install/load/enable 手順と daemon 管理
