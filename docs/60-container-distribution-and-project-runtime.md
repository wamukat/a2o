...
- TODO-4: branch-native publish / merge
  ...
  - 2026-04-12 時点で implementation publish と native merge job の Go agent path は実装済み。残りは runtime smoke を `--merge-runner agent-http` に切り替えて parent-child / single live merge の実チケット証跡を固定することと、移行完了後に Ruby `LocalWorkspaceChangePublisher` / `LocalMergeRunner` の direct/local compatibility を主経路から削除または明確に隔離すること
