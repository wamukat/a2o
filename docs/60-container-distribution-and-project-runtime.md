...
- 未実装 / 未完
  ...
  - 実 Portal source の `repo:both` parent/full verification validation は完了済み。今後は同 validation を regression validation として維持する
  - `.work` / artifact / logs / blocked diagnosis evidence retention の長時間運用検証

したがって、2026-04-12 時点の次優先は新しい trigger や検証専用概念を増やすことではない。A3 product/runtime 側で得た validation evidence と manual loop runbook を前提に、SoloBoard 前提の配布導線と retention/hardening を閉じることである。
...
A3 完成前の最終確認では、毎回重い実 Portal full verification を実行しない。通常は軽量な protocol / topology / retention validation を回し、完成判定または大きな runtime 変更時だけ実 Portal source の heavy validation を実行する。

標準バリエーション:

- `T0 runtime lifecycle`: retired Portal runtime task (up), `:bootstrap`, `:doctor` で Docker 上の A3 runtime、SoloBoard latest image、published agent port を確認する
- `T1 host-local agent protocol`: retired Portal runtime task (host-agent-validation) で Docker 上 A3 control plane と host local `a3-agent` の pull / command execution / artifact upload / result submit を確認する
- `T2 host-local worker / command gateway`: runtime run-once で implementation / verification / merge job が host local `a3-agent` 経由で A3 に戻ることを確認する
- `T3 host-local parent topology`: parent / child relation、repo:both slot、parent verification、merge が host local `a3-agent` 経由で `Done` まで進むことを retired Portal runtime task (host-agent-parent-topology-validation) で確認する
- `T5 retention / recovery`: retired Portal runtime task (agent-artifact-cleanup) dry-run, retired Portal runtime task (observe), archived blocked diagnosis の `show-run` / `show-blocked-diagnosis` で診断可能性と disk pressure 対応を確認する
- `T6 real source single / parent verification`: project source / toolchain 変更時または PR 前の重点確認として、runtime run-once による実 Portal source ticket を実行する
- `T7 real source repo:both parent full verification`: 完成判定または release candidate 判定で 1 回実行する heavy validation とし、日常 validation の必須条件にはしない

完成条件は `T0` から `T5` が host-local agent を主経路にした軽量 validation として安定し、`T6` / `T7` を必要タイミングで通せる運用証跡があることとする。`T7` は「A3 が実 Portal source を扱えるか」の最終証跡であり、「毎回実行する回帰テスト」ではない。

runtime / agent loop の運用ルール:

- 起動前に `a3-agent doctor --engine <control-plane-url> --token-file <path>` と Engine-managed agent environment doctor を実行し、control-plane URL、workspace root、source paths、required bins を検証する
- agent 起動は `a3-agent --engine <control-plane-url> --token-file <path> --loop --poll-interval 2s` を標準とする。短時間 validation のみ `--max-iterations` を使う
- Engine loop 起動は compose-managed `a3-runtime` container process とする。host OS の service manager へ A3 が直接登録しない
- 停止は retired Portal runtime task (down) または loop process の graceful stop を標準とする。強制 kill 後は retired Portal runtime task (describe-state) と retired Portal runtime task (watch-summary) で active / queued / blocked を確認する
...
- runtime run loop の通常入口は retired Portal runtime task (run-once) とし、root-local `pause-scheduler` / `resume-scheduler` / `show-scheduler-state` task は maintenance-only internal surface として通常 `task --list` から隠す。runtime 標準は operator terminal での one-shot / manual loop 起動であり、OS service 化は current scope 外である。macOS LaunchAgent 用の `a3:portal:scheduler:install/uninstall/reload/status` と `scripts/a3/launchd.rb` は削除済みで、`a3-agent` の scheduler 登録機能としては扱わない

この分類により、「operator が日常的に使う入口」は root Taskfile の runtime task に寄せ、A3 CLI の低レベル command は validation harness / diagnosis / recovery の実装面として扱う。

命名上の残課題: `bundle` は実装由来の呼称であり、operator には「Docker 上 A3 runtime + SoloBoard + host/dev-env agent を使う local runtime」という意味が伝わりにくい。A3 Engine 自体は Docker 上で動かすことを主眼に置くため、operator-facing な `runtime` は自動的に `A3 Engine on Docker` を指すものとして扱う。したがって root Taskfile の public entrypoint は `a3:portal:bundle:*` から retired Portal runtime Taskfile namespace へ rename する。移行時は短期間だけ旧名を maintenance alias として残し、docs / operator logs / watch-summary の表記から `bundle` を順次外す。

命名判断:

- 採用: retired Portal runtime Taskfile namespace
...
- 次は `SoloBoard 前提の配布導線` と `manual loop / retention hardening` を進める。旧 backend compatibility path は current runtime から削除済みであり、実 Portal source の `repo:both` parent/full verification validation は regression validation として維持する
