...
- 未完了
  ...
  - SoloBoard upstream latest の更新取り込み確認。既定 image は `ghcr.io/wamukat/soloboard:latest` のため設定変更は不要だが、次回作業で `docker compose pull soloboard` / recreate 後に bootstrap・smoke・実チケット canary を通し、新版 API 互換と read-after-write 挙動を確認する
