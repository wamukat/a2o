...
- 完了済み
  ...
  - A3 scheduler surface (`scheduler:run-once` / `scheduler:install` / `scheduler:reload` / `scheduler:status`) を workspace root から起動できる
  ...
  - 2026-04-12 に SoloBoard upstream latest を再 pull/recreate し、standalone と bundle の両方が image digest `sha256:e77c0ebcd4b49aed28ce1e97d89b2751f20249f438a870b874dbd1e1365e4191` / version `0.9.2` / revision `c5cd3f10a2c357e4d6b2e4328b2910e222307eda` で起動することを確認した。`task soloboard:doctor` / `soloboard:bootstrap` / `soloboard:smoke`、`task a3:portal:bundle:doctor` / `bundle:bootstrap` / `bundle:smoke`、`task a3:portal:bundle:agent-smoke` は成功しており、SoloBoard latest の API surface と A3 agent 接続は現行 adapter と互換である
  ...
  - Portal dev-env agent image は Java 25 / Ruby / Task / git / Python / Node/npm を持ち、`task a3:portal:bundle:agent-full-verification-smoke` で実 `member-portal-starters` repo の remediation + `task test:nullaway` を agent-http command runner 経由で実行し、SoloBoard synthetic task を `Merging -> Done` まで確認済み。`task a3:portal:bundle:agent-ui-verification-smoke` でも実 `member-portal-ui-app` repo の remediation + `task test:nullaway` を agent-http command runner 経由で実行し、support starter install を挟んで `Merging -> Done` まで確認済み。fresh Maven repo では Surefire `argLine` の Mockito javaagent が自動解決されないため、Portal verification entrypoint が `mockito-core` を明示 prefetch する
  ...
  - `task a3:portal:bundle:agent-real-parent-full-verification-smoke` で実 Portal live repo の `repo:both` parent/full verification canary を完走した。`Portal#88` parent と `Portal#89/#90` child を作成し、実 `member-portal-starters` / `member-portal-ui-app` live repo に parent smoke commit を作成した上で、starters 側 `gate:standard` / `test:all` / `test:nullaway` と ui-app 側 `gate:standard` / `test:nullaway` を agent-http 経由で実行し、parent merge が両 repo の `feature/prototype` を `refs/heads/a3/parent/Portal-88` へ更新することまで確認した
- 未完了
  ...
  - terminal task の workspace / artifact / log cleanup は operator command と scheduler idle cleanup まで実装済み。agent artifact store は diagnostic/evidence 別に TTL + count + size cap cleanup まで実装済み。2026-04-12 に bundle artifact cleanup dry-run と host cleanup dry-run を実行し、どちらも候補 0 の idle 状態を確認した。残りは logs / blocked diagnosis evidence の retention 仕様を長時間運用で実測すること
...
### 0.1.0 Bundle 実チケット smoke 証跡

2026-04-11 時点で、A3 runtime container、SoloBoard container、Portal dev-env `a3-agent` container を組み合わせた compose bundle は、実際の SoloBoard task を作成して次の smoke を通している。

- `task a3:portal:bundle:smoke`
  - `Portal#35` parent と `Portal#36` child を作成し、relation/comment/transition を含む compatibility surface を確認した
- `task a3:portal:bundle:agent-worker-gateway-smoke`
...
- `task a3:portal:bundle:agent-verification-smoke`
...
- `task a3:portal:bundle:agent-parent-topology-smoke`
...
- `task a3:portal:bundle:agent-real-parent-full-verification-smoke`
  - 初回は Portal dev-env agent image に `ripgrep` がなく、starters knowledge catalog build が `FileNotFoundError: rg` で失敗したため、`docker/a3-portal-agent/Dockerfile` に `ripgrep` を追加した
...
- `task a3:portal:bundle:observe`
  - bundle の反復観測入口として追加した
  - 各 iteration で UTC 時刻、host disk、Docker reclaimable、bundle doctor、watch-summary、show-state を `.work/a3/portal-bundle/observation.log` へ記録する
  - 観測中の command failure は `pipefail` で task failure として扱い、ログだけ成功扱いにしない
- `task a3:portal:bundle:archive-state`
  - bundle canary storage に古い blocked / failed run が残っている場合、削除ではなく `/var/lib/a3/archive/<storage-name>-<UTC timestamp>` へ退避してから空の state を作る
...
- `task a3:portal:bundle:agent-loop`
  ...
  - 各 iteration の前後で disk / Docker reclaimable / bundle doctor / watch-summary / show-state を `.work/a3/portal-bundle/agent-loop.log` へ記録する

この smoke 群により、少なくとも `docker:a3 + docker:soloboard + docker:dev-env(a3-agent)` の local compose 形状では、task 作成、relation、worker gateway、verification command runner、parent topology、artifact upload の最小経路が実チケット上で成立している。
...
- この結果を受け、`portal_remediation.rb` / `portal_verification.rb` は slot cwd から workspace root を解決できるよう修正したうえで、`a3:portal:bundle:run-once` は `A3_PORTAL_BUNDLE_ALLOW_DIRECT_RUN_ONCE=1` がない限り fail-fast するようにした
...
注意: `task a3:portal:bundle:run-once` は legacy direct-path diagnosis 用であり、通常の bundle 検証には使わない。A3 runtime container には project 固有 Java/Maven runtime を持たせない方針のため、direct path で Portal verification へ進めると `portal_remediation.rb` / repo-local `task fmt:apply` などが docker:a3 内で実行され、`JAVA_HOME` 不足などで blocked になりうる。worker/verification/parent topology の検証は agent smoke 群を正規経路とする。
...
2026-04-12 時点の配布 runbook は、`docker:a3 + docker:soloboard + a3-agent` を標準形状として扱う。A3 container は汎用 control plane であり、Portal 専用 JDK / Maven / Node runtime を持たない。project command は host または project dev-env container に install した `a3-agent` が担当する。Portal workspace での最終確認は、まず `A3 Engine on Docker` + `A3 Agents on Local(~/workspace/mypage-prototype)` を標準対象とし、dev-env container agent は同じ protocol が container runtime でも成立することを確認する補助形状として扱う。
...
4. agent 疎通確認
   ...
   - `task a3:portal:bundle:host-agent-smoke`
   - `task a3:portal:bundle:agent-worker-gateway-smoke`
   - `task a3:portal:bundle:agent-verification-smoke`
   - `task a3:portal:bundle:agent-parent-topology-smoke`
5. 反復観測
   - `ITERATIONS=<n> INTERVAL_SECONDS=<sec> task a3:portal:bundle:agent-loop`
   - `ITERATIONS=<n> INTERVAL_SECONDS=<sec> task a3:portal:bundle:observe`
   - disk / Docker reclaimable / watch-summary / show-state は `.work/a3/portal-bundle/*.log` に残す
6. retention / cleanup
   - stale internal state は削除ではなく `task a3:portal:bundle:archive-state` で `/var/lib/a3/archive/...` へ退避する
   - agent artifact store は `task a3:portal:bundle:agent-artifact-cleanup -- --dry-run` で候補確認し、必要時だけ `--apply` する
...
- `T0 bundle lifecycle`: `task a3:portal:bundle:up`, `:bootstrap`, `:doctor` で Docker 上の A3 runtime、SoloBoard latest image、published agent port を確認する
- `T1 host-local agent protocol`: `task a3:portal:bundle:host-agent-smoke` で Docker 上 A3 control plane と host local `a3-agent` の pull / command execution / artifact upload / result submit を確認する
- `T2 host-local worker gateway`: implementation worker result が host local `a3-agent` の agent-http gateway 経由で A3 に戻ることを確認する。既存の Docker dev-env equivalent は `task a3:portal:bundle:agent-worker-gateway-smoke`
- `T3 host-local verification runner`: remediation / verification command が host local `a3-agent` の job として実行され、log / artifact が A3-managed store へ残ることを確認する。既存の Docker dev-env equivalent は `task a3:portal:bundle:agent-verification-smoke`
- `T4 host-local parent topology`: parent / child relation、repo:both slot、parent verification、merge が host local `a3-agent` 経由で `Done` まで進むことを確認する。既存の Docker dev-env equivalent は `task a3:portal:bundle:agent-parent-topology-smoke`
- `T5 retention / recovery`: `task a3:portal:bundle:agent-artifact-cleanup -- --dry-run`, `task a3:portal:bundle:observe`, archived blocked diagnosis の `show-run` / `show-blocked-diagnosis` で診断可能性と disk pressure 対応を確認する
- `T6 real source single verification`: `task a3:portal:bundle:agent-full-verification-smoke` と `task a3:portal:bundle:agent-ui-verification-smoke` は project source / toolchain 変更時または PR 前の重点確認で実行する
- `T7 real source repo:both parent full verification`: `task a3:portal:bundle:agent-real-parent-full-verification-smoke` は完成判定または release candidate 判定で 1 回実行する heavy canary とし、日常 smoke の必須条件にはしない
...
- 停止は operator terminal の `Ctrl-C` を標準とする。強制 kill 後は `task a3:portal:bundle:describe-state` と `task a3:portal:bundle:watch-summary` で active / queued / blocked を確認する
...
```bash
task a3:portal:bundle:doctor
task a3:portal:bundle:watch-summary
task a3:portal:bundle:describe-state
ITERATIONS=3 INTERVAL_SECONDS=30 task a3:portal:bundle:observe
```

disk pressure 時の cleanup 判断:

- まず `df -h .` と `docker system df` で host disk と Docker reclaimable を見る
- A3 internal state は消さず、必要なら `task a3:portal:bundle:archive-state` で archive へ退避する
- agent artifacts は `task a3:portal:bundle:agent-artifact-cleanup -- --dry-run` で候補確認し、diagnostic / evidence の TTL・count・size cap を指定して `--apply` する
...
- `task a3:portal:bundle:run-once` は通常検証に使わない。これは legacy direct-path diagnosis 用であり、明示 opt-in がない場合は fail-fast する
...
2. `scripts/a3/a3_stdin_bundle_worker.rb` に executor profile resolver を追加する
3. current `a3-engine` / root tests に
   - default profile
...
#### 6.2.0 Bundle 後始末と disk pressure 時の操作

SoloBoard bundle は Docker volume に runtime state を置くため、host `.work` 配下を掃除しても bundle 内の `/var/lib/a3` や SoloBoard data volume は回収されない。bundle 検証後の後始末は、host workspace cleanup と Docker cleanup を分けて扱う。

通常の検証後:

- bundle を停止する場合は root で `task a3:portal:bundle:down` を使う
- `down` は container を止めるための操作であり、SoloBoard / A3 runtime の volume を消す操作としては扱わない
- 再度検証する場合は `task a3:portal:bundle:up`, `task a3:portal:bundle:doctor`, `task a3:portal:bundle:bootstrap` の順に戻す
- bundle canary storage の古い blocked が観測を汚す場合は、削除ではなく `task a3:portal:bundle:archive-state` で退避してから観測を再開する
- 状態を一定間隔で残す場合は `ITERATIONS=3 INTERVAL_SECONDS=30 task a3:portal:bundle:observe` を使う

artifact store の掃除:

- まず `task a3:portal:bundle:agent-artifact-cleanup -- --dry-run` で削除候補を確認する
...
- image まで削る場合は `docker image prune` を使う。ただし次回 bundle 起動時に `ghcr.io/wamukat/soloboard:latest` や A3 runtime image の再 pull/build が必要になる
