...
- 完了済み
  ...
  - `task a3:portal:bundle:agent-real-parent-full-verification-smoke` で実 Portal source の `repo:both` parent/full verification canary を完走した。`Portal#79` parent と `Portal#80/#81` child を作成し、実 `member-portal-starters` / `member-portal-ui-app` checkout を parent integration ref へ materialize した上で、starters 側 `gate:standard` / `test:all` / `test:nullaway` と ui-app 側 `gate:standard` / `test:nullaway` を agent-http 経由で実行し、親子とも `Done` まで到達した
- 未完了
...
- `task a3:portal:bundle:agent-real-parent-full-verification-smoke`
  ...
  - 再実行した `Portal#79` parent と `Portal#80/#81` child は、実 Portal source の `repo:both` parent/full verification と parent merge を agent-http / materialized workspace 経由で完了し、親子とも `Done` へ到達した
...
したがって、2026-04-11 時点の次優先は新しい trigger や検証専用概念を増やすことではない。A3 product/runtime 側で得た canary evidence を前提に、SoloBoard 前提の配布導線、agent manual loop runbook、retention/hardening を閉じることである。
