...
- 未実装 / 未完
  - `a3-agent` の production daemon 管理 / auth / 長時間運用 hardening。service 化の入口として loop mode、service-manager template rendering、local installer からの opt-in template install と load/enable hook は実装済みで、production 配布 package 側の install policy 固定が残る
