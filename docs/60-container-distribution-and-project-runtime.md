...
- 未完了
  ...
  - root cleanup は current scheduler quarantine / results / logs の age+count+size retention と disposable cache の age+size retention まで拡張済みで、scheduler idle 後の terminal workspace cleanup も自動連携済み。`.work` 全体棚卸しと project-local build output の budget 化は未完
