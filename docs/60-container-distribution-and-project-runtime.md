...
補足: agent auth の現行 contract は token file も含む。service manager / container runtime では `A3_AGENT_TOKEN_FILE` / `--agent-token-file` / profile `agent_token_file` を優先し、process arguments へ token を露出させない。
