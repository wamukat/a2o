...
- 完了済み
  ...
  - Portal dev-env agent image は Java 25 / Ruby / Task / git / Python / Node/npm を持ち、`task a3:portal:runtime:agent-full-verification-smoke` で実 `member-portal-starters` repo の remediation + `task test:nullaway` を agent-http command runner 経由で実行し、SoloBoard synthetic task を `Merging -> Done` まで確認済み。`task a3:portal:runtime:agent-ui-verification-smoke` でも実 `member-portal-ui-app` repo の remediation + `task test:nullaway` を agent-http command runner 経由で実行し、support starter install を挟んで `Merging -> Done` まで確認済み。fresh Maven repo では Surefire `argLine` の Mockito javaagent が自動解決されないため、Portal verification entrypoint が `mockito-core` を明示 prefetch する
...
- TODO-3: dedicated branch worktree materialization
  - Ruby local backend と Go agent materializer は正規経路を `worktree_branch` / dedicated branch checkout にする
...
- TODO-4: branch-native publish / merge
  - implementation publish は branch worktree 上の通常 commit とし、`git update-ref` 後同期を不要にする
  - child merge は parent branch worktree、single/parent merge は live target branch worktree 上の normal Git merge とする
...
- `task a3:portal:runtime:agent-loop`
...
4. agent 疎通確認
   ...
   - `task a3:portal:runtime:agent-parent-topology-smoke`
5. 反復観測
   - `ITERATIONS=<n> INTERVAL_SECONDS=<sec> task a3:portal:runtime:agent-loop`
...
- `T2 host-local worker gateway`: implementation worker result が host local `a3-agent` の agent-http gateway 経由で A3 に戻ることを確認する。既存の Docker dev-env equivalent は `task a3:portal:runtime:agent-worker-gateway-smoke`
- `T3 host-local verification runner`: remediation / verification command が host local `a3-agent` の job として実行され、log / artifact が A3-managed store へ残ることを確認する。既存の Docker dev-env equivalent は `task a3:portal:runtime:agent-verification-smoke`
- `T4 host-local parent topology`: parent / child relation、repo:both slot、parent verification、merge が host local `a3-agent` 経由で `Done` まで進むことを確認する。既存の Docker dev-env equivalent は `task a3:portal:runtime:agent-parent-topology-smoke`
...
- `T6 real source single verification`: `task a3:portal:runtime:agent-full-verification-smoke` と `task a3:portal:runtime:agent-ui-verification-smoke` は project source / toolchain 変更時または PR 前の重点確認で実行する
- `T7 real source repo:both parent full verification`: `task a3:portal:runtime:agent-real-parent-full-verification-smoke` は完成判定または release candidate 判定で 1 回実行する heavy canary とし、日常 smoke の必須条件にはしない
...
- `task a3:portal:runtime:run-once` は通常検証に使わない。これは legacy direct-path diagnosis 用であり、明示 opt-in がない場合は fail-fast する
...
- agent 正規経路 smoke: `task a3:portal:runtime:agent-smoke`, `:host-agent-smoke`, `:agent-worker-gateway-smoke`, `:agent-verification-smoke`, `:agent-parent-topology-smoke`, `:agent-full-verification-smoke`, `:agent-ui-verification-smoke`, `:agent-real-parent-full-verification-smoke`
...
Portal runtime は `scripts/a3/config/portal/launcher.json` を executor 正本として読み続ける。ただし、A3 Engine が `codex exec --json`、`--model`、`model_reasoning_effort` のような Codex CLI 固有語彙を解釈する形は v1 完成条件として不十分である。
...
1. `scripts/a3/config/portal/launcher.json` と `portal-dev/launcher.json` を `kind: command` へ移行する
...
- 回収済み:
  ...
  - `scripts/a3/config/portal/launcher.json` と `scripts/a3/config/portal-dev/launcher.json` は `kind: command` と command argv template へ移行済み
  ...
  - `scripts/a3/config/portal/launcher.json` の `/Users/takuma/.codex/notify.sh` 通知 hook は削除済み
