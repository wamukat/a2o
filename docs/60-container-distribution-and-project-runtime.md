...
- 未実装 / 未完
  ...
  - Kanboard compatibility path の撤去または historical path 化
...
- 次は `Kanboard compatibility path の historical 化`、`SoloBoard 前提の配布導線`、`agent service / retention hardening` を進める
...
この時点で `A3-v2#3160` の acceptance は満たしており、compatibility 資産の扱いは「retire したもの」「delete 済みのもの」「current root utility を支えるため keep するもの」に分かれた。以後の残件は、Kanboard compatibility path を historical 扱いへ下げることと、A3 / SoloBoard / a3-agent 配布導線を固定することである。
