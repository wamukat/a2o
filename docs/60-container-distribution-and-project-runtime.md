...
- 未完了
  ...
  - terminal task の workspace / artifact / log cleanup は operator command と scheduler idle cleanup まで実装済み。agent artifact store は diagnostic/evidence 別に TTL + count + size cap cleanup まで実装済み。残りは logs / blocked diagnosis evidence の retention 仕様を長時間運用で検証すること
...
- 未実装 / 未完
  - `a3-agent` の production daemon 管理 / 長時間運用 hardening。service 化の入口として loop mode、service-manager template rendering、local installer からの opt-in template install と load/enable hook は実装済み。auth も shared bearer token の最小 contract は実装済みで、TLS / authorization scope / token rotation など production security policy 固定が残る
  - SoloBoard 前提の bootstrap / task 作成 / tag / lane 整備の配布導線固定
  - Kanboard compatibility path は current default から外し、`KANBAN_BACKEND=kanboard` / `kanboard:*` の historical compatibility path として扱う方針に固定済み。残りは不要になった時点で物理削除するかの判断だけである
  - `.work` / artifact / logs / blocked diagnosis evidence retention の長時間運用検証

したがって、2026-04-11 時点の次優先は新しい trigger や検証専用概念を増やすことではない。A3 product/runtime 側で得た canary evidence を前提に、SoloBoard 前提の配布導線、agent service 化、retention/hardening を閉じることである。

### 0.1.1a 配布 runbook の current draft

2026-04-11 時点の配布 runbook は、`docker:a3 + docker:soloboard + a3-agent` を標準形状として扱う。A3 container は汎用 control plane であり、Portal 専用 JDK / Maven / Node runtime を持たない。project command は host または project dev-env container に install した `a3-agent` が担当する。
...
3. agent 設置
   ...
   - service 化する場合は `INSTALL_SERVICE=1` で template を配置し、`ENABLE_SERVICE=1` は operator が明示した場合だけ使う
4. agent 疎通確認
...
- 次は `SoloBoard 前提の配布導線`、`agent service / retention hardening` を進める。Kanboard は historical compatibility path として明示 opt-in に下げた
...
この時点で `A3-v2#3160` の acceptance は満たしており、compatibility 資産の扱いは「retire したもの」「delete 済みのもの」「current root utility を支えるため keep するもの」に分かれた。Kanboard compatibility path は current default から外し、historical baseline 観測用の明示 opt-in として残す。以後の残件は、A3 / SoloBoard / a3-agent 配布導線を固定することである。
...
したがって、step 1 から step 5b と isolated canary の single / parent-child 完走までは確認済みであり、残る main work は「未使用 command contract の parity 確認」「長期 scheduler-loop 運用と read-after-write hardening」「Docker/runtime packaging」である。Kanboard compatibility path は historical baseline 観測用の明示 opt-in として固定済みである。
...
- historical compatibility entrypoints to remove or archive before freeze
...
- `a3-agent` の Go single binary service template を production 配布 package で install/load/enable する手順と daemon 管理
