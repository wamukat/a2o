...
- TODO-4a: AI-assisted merge conflict recovery
  ...
  - implementation slices:
    - Slice 1: Go `a3-agent` merge job が content conflict 時に merge workspace を cleanup せず、conflict files / merge log / before heads を `merge_recovery_candidate` evidence として返す
    - Slice 2: domain / run-state transition に merge lane の `retryable` outcome を追加し、recoverable conflict が terminal blocked に吸われず同じ merge run の recovery lane へ進めることを固定する
    - Slice 3: Ruby `AgentMergeRunner` / scheduler が recoverable conflict を検知し、同じ merge run の recovery lane として AI worker job を enqueue できる
    - Slice 4: AI worker result を recovered merge workspace に適用し、marker scan / changed-files guard / merge commit / compare-and-swap publish を Go agent 側で実行する
