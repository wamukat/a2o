...
- 未完了
  ...
  - SoloBoard bundle の full verification canary は、implementation までは container 内で通過済みだが、`portal_v2_verification.rb` の `task test:nullaway` が Maven/Surefire dependency local repo の `mockito-core-5.17.0.jar` 解決不整合で blocked になった。bundle 配布 smoke と Portal full verification を分け、Maven dependency cache / Docker daemon / retry policy を棚卸ししてから full canary を完了させる
