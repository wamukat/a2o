...
- 完了済み
  ...
  - Portal dev-env agent image は Java 25 / Ruby / Task / git / Python / Node/npm を持ち、`task a3:portal:bundle:agent-full-verification-smoke` で実 `member-portal-starters` repo の remediation + `task test:nullaway` を agent-http command runner 経由で実行し、SoloBoard synthetic task を `Merging -> Done` まで確認済み。fresh Maven repo では Surefire `argLine` の Mockito javaagent が自動解決されないため、Portal verification entrypoint が `mockito-core` を明示 prefetch する
- 未完了
