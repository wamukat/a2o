...
- TODO-4: branch-native publish / merge
  ...
  - 2026-04-13 時点で implementation publish と native merge job の Go agent path は実装済み。Ruby Engine 側の direct project repo mutation 実装は削除し、未設定時は `DisabledWorkspaceChangePublisher` / `DisabledMergeRunner` で fail closed にする。残りは runtime smoke を `--merge-runner agent-http` に切り替えて parent-child / single live merge の実チケット証跡を固定すること
