...
この文書は、A2O を Docker コンテナとして配布し、案件ごとに利用可能にするための配布モデルと runtime packaging を定義する。内部実装名は A3 のまま維持する。公開名称と内部実装名の境界は [92-a2o-public-branding-boundary.md](92-a2o-public-branding-boundary.md) を参照する。
...
- 状態
  - `a3-engine` live validation を 1 本完了し、A3 direct verification では `scratch` / `local-live` / `parent-child` の 3 類型で `To do -> Done` の正規フローを確認済み
  - 2026-04-15 時点で、A3 Engine Docker runtime image + host-local `a2o-agent` + SoloBoard + live repo merge の release-candidate validation を完了した。isolated runtime `a3-validation-starters-remediation2` で `Portal#1` parent / `Portal#2` child を作成し、`repo:starters` parent-child flow が child implementation、child verification、child-to-parent merge、parent review、parent verification、live repo merge まで完走した。`member-portal-starters feature/prototype` は `c8a90bd0` (`A3 implementation update for Portal#2`) へ進み、両 ticket は `Done` になった
- 完了済み
  - runtime package descriptor の主要 contract を実装済み
  - `doctor-runtime` / `show-runtime-package` / `run-runtime-validation` で inspection を確認可能
  ...
  - A3 runtime run-once surface (retired Portal runtime task (run-once)) を workspace root から起動できる。historical scheduler control は maintenance-only とし、macOS LaunchAgent service surface は current scope 外として削除済み
  - reference project で `plan-run-once` selectable な live handoff validation を 1 本通し、implementation / review / inspection / merge / live repo 反映まで確認済み
  ...
  - runtime run loop 前に external kanban snapshot と reconcile する Kanban-first 運用を実装済み
  ...
  - single-repo / multi-repo / parent-child の local-live diff validation を A3 で完走し、local `feature/prototype` への実差分反映まで確認済み
  - Portal fresh-5 validation (`Portal#3156/#3157/#3158`) を A3 で完走し、child から parent finalize まで `Done` を確認済み
  - Portal direct baseline validation (`Portal#3170/#3171/#3172`) を完走し、child merge 後に parent が review / verification / merge へ handoff されることを確認済み
  - Portal runtime run-once baseline validation (`Portal#3173/#3174/#3175`) を完走し、runtime run-once 経路でも child 2 件と parent 1 件が `Done` まで進むことを確認済み
  - Portal release-candidate validation (`Portal#1/#2` on isolated SoloBoard port 3503) で、agent publish policy の実差分 publish、parent review、parent verification、live repo merge を確認済み。初回 child implementation では worker result の declared file と実 worktree 差分がずれたため blocked になったが、`a3-engine` commit `6aa4100` で edit-target 実差分 publish へ修正した。初回 parent review は unsupported model `gpt-5-codex` で blocked になったが、root commit `31bc05e` で Portal launcher model を `gpt-5.3-codex-spark` へ更新し、rerun で完走した
  - historical host launcher `a3 runtime loop` は Engine-managed runtime cycle の検証用入口として追加済みだったが、current public lifecycle では `a2o project bootstrap` / `a2o kanban ...` / `a2o agent ...` と `a2o-agent` を正規入口にする。`run-once` interval 実行は internal flow / maintenance validation の文脈に限定する
  - local release image equivalent `a3-portal-bundle-a3-runtime` から `a2o host install` し、bootstrap 後に `a2o project bootstrap` / `a2o kanban up` / `a2o agent install` / `a2o kanban doctor` / `a2o kanban url` が動くことを smoke 確認済み。public launcher `a2o`、compat alias `a3`、public share dir `share/a2o`、public agent path `.work/a2o-agent/bin/a2o-agent` を確認した。smoke 用 compose project `a2o-release-smoke` と `.work/a2o-release-smoke` は停止・削除済み
  - public brand を A2O にする方針を固定し、container 内 entrypoint / host launcher / docs は `a2o` を正にする。内部互換として `a3` command、`.a3`, `A3_*`, internal state schema は維持する
  - GHCR の正式 image ref は `ghcr.io/wamukat/a2o-engine:latest` として確定し、利用者 quickstart / release plan / public branding docs の placeholder は置換済み
  - GitHub Actions workflow `publish-a2o-engine.yml` を追加し、`main` push / manual dispatch で `ghcr.io/wamukat/a2o-engine:latest` と commit SHA tag を publish する。PR では image push せず build のみ確認する
  ...
  - SoloBoard bundle spike では Portal starters の Java 25 / google-java-format 1.34.1 前提に合わせて一時的に Temurin 25 JDK を A3 runtime image へ入れたが、これは完成形ではない。最新方針では A3 image から project 固有 JDK / Maven / verification runtime を剥がし、project runtime は public `a2o-agent` 側へ分離する
  ...
  - 2026-04-11 の bundle 実チケット validation では、`Portal#37` が worker gateway 経由で `Inspection -> Done`、`Portal#39` が agent verification command runner 経由で `Merging -> Done`、`Portal#40` が child `Portal#41/#42` を集約する parent topology で `Done` まで到達した。初回 `Portal#38` は synthetic non-Maven fixture で Maven bootstrap/prefetch を走らせたため blocked になったが、`47dda1f` で non-Maven verification slot では Maven bootstrap を skip するよう修正し、復旧コメントと blocked label removal 後に superseded validation として `Done` 化した
  - retired Portal runtime task (agent-real-parent-full-verification-validation) で実 Portal live repo の `repo:both` parent/full verification validation を完走した。`Portal#88` parent と `Portal#89/#90` child を作成し、実 `member-portal-starters` / `member-portal-ui-app` live repo に parent validation commit を作成した上で、starters 側 `gate:standard` / `test:all` / `test:nullaway` と ui-app 側 `gate:standard` / `test:nullaway` を agent-http 経由で実行し、parent merge が両 repo の `feature/prototype` を `refs/heads/a3/parent/Portal-88` へ更新することまで確認した
- 未完了
  ...
  - 旧 backend contract / bootstrap / adapter / cutover validation は現行計画から外し、SoloBoard 前提に再整理する
  ...
  - 正式 registry image ref 公開後に、`a2o host install` / `a2o project bootstrap` / `a2o kanban up` / `a2o agent install` / `a2o kanban doctor` / `a2o kanban url` を registry image で再 smoke すること
  - Portal live repo を remote 最新へ戻す場合、A3 実行に必要な repo-local Taskfile / bootstrap 変更をどの commit として再投入するかを release 前整理で固定すること
...
- TODO-3: dedicated branch worktree materialization
  - Docker + host/dev-env agent mode では、project repo の worktree 作成、branch checkout、dirty check、cleanup、quarantine は `a2o-agent` が配置された runtime 側で行う
  ...
  - runtime instance ごとに branch namespace を持つ。isolated board / reset board では `Portal#1` のような task ref が再利用されるため、branch ref は `refs/heads/a3/<runtime-namespace>/work/<task>` / `parent/<task>` とし、historical `refs/heads/a3/work/<task>` を再利用しない
- TODO-4: branch-native publish / merge
  ...
  - merge は `merge_request` を正規 contract とする。A3 Engine は `--merge-runner agent-http` で merge job を control plane に投入し、Go agent が Engine-provided agent environment config の source path 上で target branch worktree を作成して normal Git merge を実行する。agent は `merge_before_head` / `merge_after_head` / `resolved_head` / `project_repo_mutator=a3-agent` を返し、Engine は `AgentMergeRunner` で検証する
  - 2026-04-13 時点で implementation publish と native merge job の Go agent path は実装済み。Ruby Engine 側の direct project repo mutation 実装は削除し、未設定時は `DisabledWorkspaceChangePublisher` / `DisabledMergeRunner` で fail closed にする。agent-materialized verification / worker / merge は Engine workspace preparation を skip し、runtime validation は `--verification-command-runner agent-http` / `--merge-runner agent-http` を明示して parent-child / single live merge の実チケット証跡を固定する
...
- TODO-4a: AI-assisted merge conflict recovery
  - deterministic merge を第一段に固定する。Go `a2o-agent` はこれまで通り dedicated merge workspace で normal Git merge を実行し、conflict しなければ AI worker を起動しない
  - normal Git merge が conflict した場合だけ、merge workspace を破棄せず `merge_recovery` candidate として保持する。保持対象は conflict marker を含む working tree、target/source ref、merge_before_head、source head、conflict file list、merge log、runtime slot descriptor とする
  - A3 Engine は merge failure を即 terminal `blocked` にせず、failure class が recoverable conflict の場合に限り merge phase 内の `merge_recovery` lane を計画する。checkout failure、dirty tree、missing ref、multi-slot partial publish、rollback failure は AI recovery に回さず従来通り blocked にする
  - `merge_recovery` は正式 phase を増やさず、merge phase 内の AI worker action として扱う。入力は conflict workspace と merge evidence に限定し、worker には「conflict marker 解消」「conflict file 原則のみ変更」「source/target 双方の意図を残す」「不要な再実装をしない」を明示する
  - AI worker が解消した後、Go agent は recovered merge workspace 上で `git status --porcelain`、conflict marker scan、changed-files guard、commit creation、target ref compare-and-swap publish を実行する。Engine は `MergeRecovery` evidence の `merge_before_head` / `source_head_commit` / `worker_result_ref` / `resolved_conflict_files` / `publish_before_head` / `publish_after_head` を検証する
  - recovery 後は必ず対象 task の verification を再実行する。verification が成功するまで Done / child-to-parent complete / live merge complete としない
  - recovery が失敗、timeout、変更範囲逸脱、未解決 conflict marker 残存、verification failure のいずれかになった場合は blocked とし、AI が作った未 publish 差分は target ref へ反映しない
  - operator 視点では、通常の merge failure comment に加えて `merge_recovery started/completed/blocked` を SoloBoard comment と watch-summary の merge lane detail に出す。手動 recovery した場合も state reconcile コマンドで live commit と A3 internal state を対応づけられるようにする
  - focused regression は Portal#245 型の validation docs 追記 conflict を fixture 化し、AI worker stub が conflict marker を解消した場合に live target が publish され、verification rerun 後に Done へ進むことを確認する
  - implementation slices:
    - Slice 1: done. Go `a2o-agent` merge job が content conflict 時に merge workspace を cleanup せず、conflict files / merge log / before heads を `merge_recovery_candidate` evidence として返す
    - Slice 2: done. domain / run-state transition に merge lane の `retryable` outcome を追加し、recoverable conflict が terminal blocked に吸われず同じ merge run の recovery lane へ進めることを固定する
    - Slice 3: done. Ruby `AgentMergeRunner` が recoverable conflict を検知し、通常 CLI 導線の `worker_command` を使って同じ merge run の recovery lane として AI worker job を enqueue し、その後 `a3-agent-merge-recovery` finalizer job を enqueue する
    - Slice 4: done. AI worker が解消した recovered merge workspace に対して、marker scan / changed-files guard / merge commit / partial publish rollback を Go agent 側で実行する。Engine は finalizer evidence の `publish_after_head == merge_after_head == resolved_head`、未解決 marker なし、conflict file 内変更を検証する
    - Slice 5: done. recovery publish 後に task-scoped `verification_source_ref` を設定し、次の verification run が published target ref を検証するようにした。`retryable` / `terminal_noop` の verification rerun でも同 ref を保持する。`show-run` は `merge_recovery` evidence を専用行で表示し、SoloBoard comment には recovery summary / target / publish head を出し、watch-summary は recovery verification source を detail 表示する
    - Slice 6: done. `reconcile-merge-recovery` command を追加し、operator が live repo に入れた commit/ref を最新かつ未解決の `merge_recovery` evidence に限定して A3 internal state と SoloBoard comment/status に対応づけ、次 verification の source ref として保持する
    - Slice 7: done. Portal#245 型 validation docs conflict を `AgentMergeRunner` fixture として固定し、single / parent merge recovery publish が live target ref を verification_source_ref として verification に戻る focused regression を追加する。サブエージェントレビューで設計思想と差分の両方を確認してから完了扱いにする
- TODO-5: parent-child workspace topology in agent mode
  ...
  - Slice 1: done. `workspace_request.topology` / `workspace_descriptor.topology` を追加し、child task の agent-materialized workspace を `parent_workspace_id/children/<child>/<workspace_kind>` 配下へ materialize する。`relative_path` は agent 側で absolute / `..` escape を拒否する
  - Slice 2: done. parent task 自体の `workspace_id` も stable parent workspace id (`<parent>-parent`) に揃え、child topology の `parent_workspace_id` と実 parent materialization root を一致させる
  - Slice 3: done. Engine-local terminal cleanup は parent workspace id (`<parent>-parent`) と child topology (`<parent>-parent/children/<child>`) を使い、parent `Done` cleanup pass で terminal child workspace も回収する。stale child_refs は child 側の `parent_ref` が一致する場合だけ信頼し、cleanup では ambiguous nested fallback を使わない
  - Slice 4: done. Go `a3-agent cleanup-workspace --workspace-root <root> --descriptor <workspace-descriptor.json> [--dry-run]` を追加し、workspace descriptor の topology から `<parent>-parent/children/<child>/<workspace_kind>` を再構成して registered worktree と workspace root を回収する。slot `runtime_path` が descriptor root 外の場合は拒否し、repo source は slot metadata (`.a3/slot.json`) の `repo_source_root` を正とする
- TODO-6: Portal Maven local repository policy
  ...
  - host-local runtime run-once は `A3_MAVEN_WORKSPACE_BOOTSTRAP_MODE=empty` を agent job へ注入し、scoped local repo は作るが shared seed の全コピーは行わない。これにより starter install artifact の task/parent scoped isolation は維持しつつ、third-party cache 全量コピーによる disk pressure を避ける
  - この policy は Portal runtime から A3 へ注入する。A3 core は `member-portal-starters`、Maven groupId、`mvn install` の意味を固定知識として持たない
  - Slice 1: done. Portal injection の Maven bootstrap は workspace local repo の manifest owner (`workspace_root` / `target_path`) が現在の workspace と一致しない場合に fail-fast する。verification / remediation は `MAVEN_REPO_LOCAL` と同じ scoped repo を `A3_WORKSPACE_MAVEN_REPO` として support bootstrap へ渡し、requested path fallback ではなく workspace descriptor 由来の local repo を正とする

これらの TODO は設計メモではなく実装 backlog であり、各 TODO は実装後にサブエージェントレビューで「差分の正しさ」と「設計思想の反映」を確認してから完了扱いにする。

### 0.1.0 Runtime 実チケット validation 証跡

2026-04-11 時点では、A3 runtime container、SoloBoard container、Portal dev-env `a3-agent` container を組み合わせた compose bundleで次の validation を通していた。2026-04-13 の配布整理により、Docker dev-env agent image とその validation task は現行配布から削除し、host-local `a2o-agent` 経路を正規経路にした。以下は historical evidence として扱う。`a3-agent` はこの段落では当時の container/service 名であり、current public binary 名ではない。

- retired Portal runtime task (validation)
  - `Portal#35` parent と `Portal#36` child を作成し、relation/comment/transition を含む compatibility surface を確認した
- retired Portal runtime task (agent-worker-gateway-validation)
...
- retired Portal runtime task (agent-verification-validation)
...
- retired Portal runtime task (agent-parent-topology-validation)
...
- retired Portal runtime task (agent-real-parent-full-verification-validation)
...
- retired Portal runtime task (archive-state)
  - runtime validation storage に古い blocked / failed run が残っている場合、削除ではなく `/var/lib/a3/archive/<storage-name>-<UTC timestamp>` へ退避してから空の state を作る
  - SoloBoard の ticket/comment/relation data は変更しない
- retired Portal runtime task (agent-loop) (historical; removed)
  - agent 正規経路の反復観測入口として追加した
  - 各 iteration で `agent-worker-gateway-validation`, `agent-verification-validation`, `agent-parent-topology-validation` を順に実行し、label / transition / relation / done confirmation を含む mutation 経路を繰り返す
...
この validation 群は `docker:a3 + docker:soloboard + docker:dev-env(a3-agent)` 形状が成立していた historical evidence である。現行配布形状は `docker:a3 + docker:soloboard + host/project-dev-env a2o-agent binary` であり、`a3-agent` は内部互換名、Docker dev-env agent image は配布対象外である。
...
- その後、diagnostic validation として `Portal#43` を legacy direct `bundle:run-once` に流したところ、implementation は通ったが verification が docker:a3 内の `portal_remediation.rb` で project runtime を要求し blocked になった
...
- `Portal#43` は diagnostic validation として recovery comment を残し、blocked label を外して `Done` 化した。A3 internal validation storage は `/var/lib/a3/archive/portal-soloboard-bundle-validation-20260411T142541Z` へ退避し、再観測で `active_runs=0`, `queued_tasks=0`, `blocked_tasks=0` を確認した
...
注意: legacy direct-path diagnosis 用の旧 direct run-once task は削除済みである。A3 runtime container には project 固有 Java/Maven runtime を持たせない方針のため、Docker A3 から Portal verification を直接実行する経路は通常 surface に戻さない。worker/verification/parent topology の検証は public host-local `a2o-agent` 経路を正規経路とする。

### 0.1.1 実装済みと未実装の切り分け

2026-04-06 時点では、A3 は「core が未着手」ではなく、「core はかなり揃っているが Portal scheduler の実運用接続が未完」という状態だった。2026-04-11 時点では scheduler / SoloBoard / agent validation まで進み、残りは配布・長時間運用 hardening 側へ移っている。
...
- 実装済み
  ...
  - local-live diff validation による live repo 反映
...
- 実装済みだが長時間運用 hardening が残るもの
  ...
  - project command execution を host / dev-env 側 `a2o-agent` へ寄せる transport と artifact upload
- 未実装 / 未完
  - `a2o-agent` の長時間運用 hardening。`a2o-agent --loop --poll-interval ...` は dev-env residency / diagnostic / operator-controlled fallback として整理済みで、OS service template / install / load / enable は標準導線から外す。auth も local token file の最小 contract は実装済みで、2026-04-12 の historical bundle observe では idle / disk / Docker reclaimable が安定していることを確認した。残りは reference product suite 上で failure recovery と blocked diagnosis evidence retention を実測確認することである。中央 A3 server / remote multi-agent pool / remote TLS termination は現行スコープ外とする
  - SoloBoard 前提の bootstrap / task 作成 / tag / lane 整備の配布導線固定
  - 旧 backend compatibility path は削除する。current kanban backend は SoloBoard のみとし、過去の旧 backend validation は履歴証跡として文書にだけ残す
  - 実 Portal source の `repo:both` parent/full verification validation は完了済み。今後は Portal integration validation として別枠に維持し、A2O core regression validation は reference product suite へ移す
  - `.work` / artifact / logs / blocked diagnosis evidence retention の長時間運用検証

したがって、2026-04-16 時点の次優先は Portal を通常 regression に戻すことではない。A2O reference product suite を作り、SoloBoard 前提の配布導線と retention/hardening をその suite 上で閉じることである。

### 0.1.1a 配布 runbook

2026-04-13 時点の配布 runbook は、`docker:a3 + docker:soloboard + host/project-dev-env a2o-agent binary` を標準形状として扱う。A3 container は汎用 control plane と runtime loop の所有者であり、Portal 専用 JDK / Maven / Node runtime を持たない。project command は host または project dev-env container に install した public `a2o-agent` が担当する。A3 release に含める Dockerfile は `a3-engine/docker/a3-runtime/Dockerfile` を正本とし、A3 Engine command は image 内の `/usr/local/bin/a3` として提供する。host に Ruby interpreter や host Ruby `a3` executable を install させない。workspace root 直下の `docker/a3-agent` / `docker/a3-portal-agent` image は削除済みである。

現行 A3 は local-first runtime である。A3 / SoloBoard / public `a2o-agent` は同一端末、または同一 Docker compose / project dev-env network 内で動く前提に固定する。A3 server を中央に置き、複数端末の remote agent が接続して自動処理する構成は、現在の state / job queue / artifact store / workspace descriptor のデータ構造では前提にしていない。

標準運用は Engine-managed runtime loop とする。operator が OS service を登録するのではなく、A3 runtime container 内の 1 process が `execute-until-idle` 相当の cycle を interval 実行し、agent job queue と runtime state を管理する。public `a2o-agent` は scheduler を持たず、control plane を poll して job を実行する worker に徹する。これにより `systemd` / `launchd` / Windows Service の差分を A3 配布物から外しつつ、kanban 自動実行の所有者を A3 Engine へ戻せる。

責務境界:

- A3 Engine container: SoloBoard から runnable task を取得し、run/state/phase、agent job 発行、pause/resume、stale run reconcile、retention cleanup を管理する。
- SoloBoard container: kanban data と ticket relation/comment/status を保持する。
- `a2o-agent`: host または project dev-env で job を pull し、git worktree / branch / project command / merge / artifact upload を実行する。`a3-agent` は内部互換名としてだけ扱う。
- OS / shell: compose 起動と agent process 起動だけを担当する。service manager integration は A3 の標準配布範囲外とし、必要な利用者が外側で wrapper 化する。

配布物:

- `a3-engine` image: `/usr/local/bin/a3`, runtime loop entrypoint, agent control-plane API, SoloBoard adapter, storage/retention tooling, multi-platform `a2o-agent` release binaries を含む。
- `a2o-agent` export package: Engine image に同梱した Go single binary を `a2o agent install` で host / project dev-env へ取り出す。内部 maintenance command として `a3 agent package list/export/verify` 相当も残る。macOS / Linux / WSL2 Ubuntu を対象とし、Windows native は対象外。
- compose package: `a3-runtime` と `soloboard` を起動する。project runtime は含めない。
- project injection package: manifest / launcher config / verification helper / project-specific bootstrap config / agent environment config を含む。Portal 固有内容は A3 Engine image へ焼き込まない。

agent binary distribution:

- Engine image は release unit の正本として、Engine と同じ protocol version の `a2o-agent` binaries を `/opt/a3/agents/<target>/a2o-agent` のような場所に同梱する。内部互換名の `a3-agent` path も必要に応じて同梱する。
- 利用者は Go toolchain を install しない。host install は Engine image から binary を export する。
- target は host/dev-env の実行 OS/arch で選ぶ。Apple Silicon host で host agent を動かすなら `darwin-arm64`、Linux dev-env container 内で agent を動かすなら `linux-arm64` または `linux-amd64` になる。
- Engine は同梱 binary の checksum manifest を持ち、export 時に検証できるようにする。
- agent binary は Engine image に同梱されるが、project command 実行は Engine container ではなく export 先の host/dev-env 側で行う。

実装状態:

- `a3 agent package list/export/verify` は A3 CLI に実装済み。
- `a3-engine/docker/a3-runtime/Dockerfile` は multi-stage build で `agent-go/scripts/build-release.sh` を実行し、`/opt/a3/agents` に macOS / Linux agent archive、`release-manifest.jsonl`、`checksums.txt` を同梱する。
- export は manifest の SHA-256 を検証してから archive 内の `a2o-agent` binary を指定先へ展開し、利用者 host に Go toolchain / Ruby interpreter を要求しない。
...
docker run --rm \
  ...
  a2o host install --output-dir /install/bin --share-dir /install/share/a2o --runtime-image ghcr.io/wamukat/a2o-engine:latest

# WSL2 Ubuntu / Linux dev-env へ export
docker run --rm \
  -v "$PWD/.work/a2o-agent/bin:/out" \
  ghcr.io/wamukat/a2o-engine:latest \
  a2o agent install --target linux-amd64 --output /out/a2o-agent
```

設定管理:

- Project manifest / launcher config / kanban config / phase policy は Engine が解釈する A3 論理設定である。
- Agent environment config は Engine が保管するが、値の意味は agent 視点である。`source_paths`, `workspace_root`, command path, dev-env URL は Engine container から見た path ではない。
- Engine は agent environment config の schema と必須項目だけを検査する。path の存在、git repo かどうか、toolchain の有無は agent doctor job で検査する。
...
historical bundle 導入順は次のとおりである。これは 2026-04-13 以前の検証手順であり、現行 A2O public runbook ではない。現行の通常入口は `a2o host install`、`a2o project bootstrap`、`a2o kanban ...`、`a2o agent install`、`a2o-agent` である。

1. runtime 起動
   - retired Portal runtime task (up)
   - retired Portal runtime task (bootstrap)
   - retired Portal runtime task (doctor)
   - `doctor` が SoloBoard `/api/boards`、runtime container の `ruby` / transitional `python3` / `task` を確認する。`python3` は `tools/kanban/cli.py` subprocess adapter が runtime default である間だけ必要な依存である
2. SoloBoard 初期化
   - bootstrap は `Portal` / `OIDC` / `A3Engine` board、lane、tag を seed する
   - `task kanban:doctor` と `task kanban:validation` で generic operator surface を確認する
3. agent 設置
   - host 実行では Engine image から `a2o-agent` binary を export して install する
   - dev-env container 実行では Engine image から取り出した同じ release binary を project dev-env image へ組み込む
   - agent environment config は Engine 側 project config に置く。`workspace_root`, `source_paths`, `env`, command path は agent から見た値として記述する
   - 標準 lifecycle は `a2o host install` / `a2o project bootstrap` / `a2o kanban ...` / `a2o agent install` で扱う。`a2o-agent --engine <control-plane-url> --token-file <path> --loop --poll-interval 2s` は dev-env residency、diagnostic、operator-controlled fallback 用に限定する
   - agent loop は worker 待機であり、kanban selection / scheduler state の所有者ではない
   - OS service 化は現行完成条件から外す。必要な場合は operator が A3 の外側で agent loop command を wrapper 化する
4. agent 疎通確認
   - `a2o-agent doctor --engine <control-plane-url> --token-file <path>`
   - Engine は agent environment doctor job を発行し、agent が source path / workspace root / required bins / git repo 到達性を検査して返す
   - retired Portal runtime task (run-once)
5. runtime loop 起動
   - current implementation では retired Portal runtime task (run-once) が 1 cycle 実行であり、Engine-managed continuous loop は次スライスで追加する
   - 完成形では retired Portal runtime task (loop) が runtime container 内で interval 実行し、operator は `watch-summary` / `describe-state` / `pause` / `resume` で状態を見る
6. 反復観測
   - `ITERATIONS=<n> INTERVAL_SECONDS=<sec> <retired Portal runtime observe task>`
   - disk / Docker reclaimable / watch-summary / show-state は `.work/a3/portal-bundle/*.log` に残す
7. retention / cleanup
   - stale internal state は削除ではなく retired Portal runtime task (archive-state) で `/var/lib/a3/archive/...` へ退避する
   - agent artifact store は retired Portal runtime task (agent-artifact-cleanup) dry-run で候補確認し、必要時だけ `--apply` する
   - host `.work` や Docker build cache は operator cleanup の対象であり、SoloBoard ticket data とは分離して扱う

Engine 側 agent environment config 例:

```yaml
agents:
  default:
    runtime_profile: host-local
    workspace_root: ${PROJECT_ROOT}/.work/a3-agent/workspaces
    source_paths:
      repo_alpha: ${PROJECT_ROOT}/member-portal-starters
      repo_beta: ${PROJECT_ROOT}/member-portal-ui-app
    env:
      A3_ROOT_DIR: ${PROJECT_ROOT}
    required_bins:
      - git
      - task
      - codex
...
2026-04-16 以降の A2O core final validation は reference product suite を標準対象にする。
Portal full verification は release candidate、Portal package 変更、または実プロダクト integration 確認に限定する。
reference product suite が未実装の間は、下記 `R*` を acceptance target として扱い、Portal の historical validation を標準 validation の代替正本に戻さない。

標準バリエーション:

- `R0 runtime lifecycle`: reference product package を `a2o project bootstrap` し、`a2o kanban up` / `a2o kanban doctor` / `a2o kanban url` で Docker 上の A2O runtime、SoloBoard image、published control-plane port を確認する
- `R1 host-local agent protocol`: reference product package 上で host local `a2o-agent` の install / doctor / pull / command execution / artifact upload / result submit を確認する
- `R2 TypeScript API + Web`: `A2O#255` の package で implementation / verification / merge job が host local agent 経由で A2O に戻ることを確認する
- `R3 Go API + CLI`: `A2O#256` の package で single binary / CLI / API server の build / test / validation intent を確認する
- `R4 Python service`: `A2O#257` の package で Python service の test / build / runtime command が project package 側に閉じ、A2O runtime image へ Python toolchain を要求しないことを確認する
- `R5 multi-repo parent topology`: `A2O#258` の package で parent / child relation、repo slot、parent verification、cross-repo merge が host local agent 経由で `Done` まで進むことを確認する
- `R6 retention / recovery`: reference product suite の blocked / failed fixture を使い、artifact cleanup dry-run、runtime observe、`show-run` / `show-blocked-diagnosis` 相当で診断可能性と disk pressure 対応を確認する

完成条件は `R0` から `R6` が host-local agent を主経路にした軽量 validation として安定することとする。
...
- A3 internal state は消さず、必要なら retired Portal runtime task (archive-state) で archive へ退避する
- agent artifacts は retired Portal runtime task (agent-artifact-cleanup) dry-run で候補確認し、diagnostic / evidence の TTL・count・size cap を指定して `--apply` する
...
- legacy direct-path diagnosis 用の旧 direct run-once task は削除済み。Docker A3 から project verification を直接実行する entrypoint は戻さない
...
- A2O public kanban lifecycle: `a2o project bootstrap --package DIR`, `a2o kanban up`, `a2o kanban doctor`, `a2o kanban url`
- A2O agent install: `a2o agent install --target auto --output PATH`
- execution loop: public Taskfile entrypoint は持たず、internal runtime flow として扱う
- runtime inspection / recovery: A3 internal maintenance command として扱い、利用者向け runbook には出さない
...
- low-level phase execution commands (`run-worker-phase`, `run-verification`, `run-merge`) は direct operator command として個別実行するより、agent validation / parent topology validation / live repo full verification validation の中で確認する
- low-level run mutation commands (`start-run`, `complete-run`, `prepare-workspace`, `execute-next-runnable-task`, `execute-until-idle`, `run-runtime-validation`) は runtime internals / validation harness 用であり、通常 operator runbook の入口にはしない
- runtime run loop の通常入口は A2O public lifecycle command と reference product package で提供する。Portal runtime task は historical / maintenance-only surface とし、root-local `pause-scheduler` / `resume-scheduler` / `show-scheduler-state` task は maintenance-only internal surface として通常 operator runbook から外す。operator の通常導線は `a2o project bootstrap` / `a2o kanban ...` / `a2o agent ...` であり、direct `a2o-agent --loop --poll-interval ...` は dev-env residency、diagnostic、operator-controlled fallback の文脈に限定する。OS service 化は current scope 外である。macOS LaunchAgent 用の `a3:portal:scheduler:install/uninstall/reload/status` と `scripts/a3/launchd.rb` は削除済みで、`a2o-agent` の scheduler 登録機能としては扱わない

この分類により、「operator が日常的に使う入口」は A2O public lifecycle command に寄せ、A3 CLI の低レベル command は validation harness / diagnosis / recovery の実装面として扱う。

命名上の historical note: `bundle` は実装由来の呼称であり、operator には「Docker 上 A3 runtime + SoloBoard + host/dev-env agent を使う local runtime」という意味が伝わりにくかった。過去の Portal Taskfile namespace は historical / maintenance-only surface として扱い、A2O の current public lifecycle では `a2o project bootstrap` / `a2o kanban ...` / `a2o agent ...` と reference product package を正規入口にする。

命名判断:

- historical: retired Portal runtime Taskfile namespace
  - Portal integration / historical diagnosis 用にだけ読む
  - A2O core validation の通常実行環境としては見せない
  - current public lifecycle は A2O command と reference product package で明示する
...
live validation と scheduler surface の検証結果に加え、workspace root の operator surface / Python utility の棚卸しと engine CLI 集約を進めた結果、A3 は `legacy scheduler や root Python thin tooling がないと Portal validation を進められない` 段階を抜けた。2026-04-11 時点では `a3-v2/` source tree と legacy automation scripts も削除済みであり、現時点の未完は runtime の正当性よりも、配布・agent 運用・retention hardening の整理である。

- A3 product/runtime 側で達した状態
  - direct verification と fresh validation を A3 側の経路で完走できる
...
- Portal validation を進めるために v1/legacy を使う必要はない
...
- 次は `SoloBoard 前提の配布導線` と `a2o-agent worker / retention hardening` を進める。旧 backend compatibility path は current runtime から削除済みである。実 Portal source の `repo:both` parent/full verification validation は historical evidence と Portal integration validation として維持し、A2O core regression validation は reference product suite へ移す
...
- `51c47dc` `Fix A3 runtime validation readiness and CLI shape`
...
- `68b169e` `Finalize A3-v2 live-write validation completion flow`
...
### 0.3 Live Validation Evidence

reference project を使った thin live validation の現時点の証跡:

- validation 類型
  - single-repo live handoff validation
- confirmed
  - project validation
  - runtime doctor
  - context inspection
...
reference project の live handoff validation で観測した integration gap と、その分類:
...
ここまでで確認した live validation は、A3-v2 の設計検証に使える材料ではあるが、実行主体は `a3-engine` だった。

- `a3-engine`
  - live kanban / scheduler / phase handoff / implementation-review-inspection-merge の既存 automation 実行系
- `A3-v2`
  - runtime package / doctor / recovery / runtime validation contract を直接検証すべき product 側

したがって、以後 `A3-v2` の検証と呼ぶものは、`a3-engine` の live validation ではなく、`a3-v2/bin/a3` を直接使う。
...
- multi-repo standalone task
  - `show-runtime-package` / `doctor-runtime` / `run-runtime-validation` / `execute-until-idle` を A3-v2 から直接実行し、`To do -> Done` を確認済み
...
- product 側で閉じられる runtime package / inspection / recovery / validation contract の残差
- live validation で観測された product-side gap の是正
- 次の live validation に入れる target shape の固定
...
- `A3-v2#3031`
  - live validation / fresh-5 evidence の intake 親 task
  - 2026-04-08 時点で `Done`
- `A3-v2#3048`
...
- `A3-v2#3151`
  ...
  - 2026-04-08 時点で `Done`
- `A3-v2#3159`
  - parent validation 中の repeated workspace-local Maven bootstrap 縮減
  - 2026-04-08 時点で `Done`
...
Portal fresh validation の intake と stabilisation は `A3-v2#3031` / `#3119` / `#3150` / `#3151` / `#3159` を `Done` にしたことで一段落した。以後の主題は `Portal validation を通すこと` ではなく、`A3-v2 が v1/legacy に依存せず自立すること` へ移る。

2026-04-08 時点で、A3-v2 Project で issue 管理されている主残件は `A3-v2#3103` と `A3-v2#3160` である。`A3-v2#2949` / `#2954` / `#2960` / `#2961` は fresh-5 validation と root utility migration 後の current evidence をもって `Done` 化済みである。`#3103` は backend bootstrap の別レーン、`#3160` は compatibility 資産の retire / archive / keep 判断を担う。
...
- Python/root-wrapper -> engine CLI migration
  ...
  - `stdin bundle worker`, `direct validation worker`, root utility surface
...
- `#2954` / `#2960` / `#2961` を fresh-5 validation と root utility migration 後の current evidence で close judgment 済み
...
3. 旧 backend baseline の収束
- direct / scheduler の parent-child validation は `Portal#3170/#3171/#3172` と `Portal#3173/#3174/#3175` で確認済み
- pause/resume validation は `Portal#3179` で完了し、pause 中に current shot は next task へ進まず、resume 後に child / parent が `Done` まで再開することを確認した
- stale recovery / reconcile validation は `Portal#3180` で完了し、shot kill / active run stale 化後に `show-state` が `stale_shot_lock,stale_run` を露出し、`repair-runs --apply` と clean rerun で `Done` まで復旧できることを確認した
- `A3-v2#3103` の旧 backend 着手は、この baseline が揃うまで defer する
...
- 旧 backend baseline が未完了の間は、legacy automation scripts と `A3Engine-v1` は参照用にだけ残し、物理削除や repo wipe には進まない
- baseline exit 条件は `Portal#3179` を含む pause/resume validation 完了と stale recovery / reconcile validation 完了である
...
- docs / kanban / operator 文言
  ...
  - `v2` を前提にした phase / validation / cutover の説明
...
要するに、`legacy/v1 を削除するか` の判断と `A3Engine を新 repo として作り直すか` の判断は切り離さず、旧 backend baseline 完了後に `A3-v2 -> A3Engine` への naming cutover まで含めて一体で進める。

要するに、次の実装優先順位は `root utility の言語移植` ではなく、旧 backend baseline を閉じてから `A3-v2#3103` を再開できるかを判断し、そのうえで `legacy/compatibility 資産の削減判断` を進めることである。
...
3. Portal fresh validation で自動実行の再検証
- fresh Portal task を使い、A3-v2 scheduler で `To do -> Done` を再度確認する
- 検証対象はまず cross-repo parent/child validation を優先する
...
この計画では、先に `A3-v2#3151` / `#3119` / `#3150` で fresh rerun と verification gate の正当性を回復し、その後の fresh Portal validation evidence を使って `2954` / `2960` / `2961` / `2949` を閉じる。設計残差を chat 上の印象で閉じず、Portal fresh validation の evidence で closing judgment を行うことを原則とする。
...
- launchd env の runtime completeness
  ...
  - v2 は初期状態で legacy kanban API env を launchd env file へ出しておらず、常駐 scheduler が起動直後に落ちる後退が発生した
- build / gate lane の分離
...
| launchd env completeness | launcher config から Kanban 接続情報を env file へ出す | 初期実装で legacy kanban API env を出していなかった | scheduler が API token missing で即死 | env completeness を runtime contract として spec 化し、prepare script を正本化する |
...
Portal live validation は、以後 `新機能実験` ではなく `安定化フェーズ` として扱う。個別 bug を見つけて都度直すのではなく、P0/P1 の受け入れ条件を満たすまで「scheduler として信頼できる」とは扱わない。

固定 rule:

- P0 が揃うまでは、新しい lane や project 固有機能を追加しない
- validation で新しい failure mode が出た場合、次の validation を流す前に regression test を追加する
- `blocked` / `idle` / `running` / `stale` の判定は operator surface を正本とし、個別 log 読みの場当たり判断に戻らない
- product task の進行と A3-v2 基盤改修を同じ成功指標で混ぜない
- `Done` になった validation task があっても、P0 acceptance を満たさなければ A3-v2 の品質安定とは判定しない
...
#### 0.4.4.2.4 validation exit criteria

安定化フェーズを抜ける条件は、単一の成功 task ではなく、次の acceptance validation 群を連続で通すことで判定する。
...
- 上の 4 validation が連続で成功すること
- 各 validation の failure mode が regression test 化されていること
- operator が manual filesystem cleanup なしで復旧できること
- `Portal` validation を 1 本成功させた、だけでは exit と見なさない
...
- live validation は直前 step が green のときだけ回す

禁止:

- regression test なしで validation failure を「既知」として先送りすること
...
2026-04-10 時点で、A3 Engine の historical kanban backend baseline は旧 backend だった。`Portal` 上の旧 backend baseline validation は `Portal#3170/#3171/#3172`, `Portal#3173/#3174/#3175`, `Portal#3179`, `Portal#3180` で完了しており、backend 非依存 contract の過去証跡として文書に保持する。2026-04-12 の判断で current runtime の旧 backend compatibility path は削除し、`task kanban:*` と `a3-engine/tools/kanban/kanban_cli.py` は SoloBoard 専用に固定する。

旧 backend baseline の current evidence:
...
- stale recovery / reconcile validation
  - `Portal#3180` で shot kill / stale run 露出 / `repair-runs --apply` / clean rerun を通し、`Done` まで復旧した

したがって、旧 backend baseline は「切替前の完了証跡」として残すが、実行可能な compatibility path は残さない。今後の課題は SoloBoard 専用 runtime の長期運用 hardening と Docker/runtime packaging を固定し、実 Portal source の `repo:both` parent/full verification validation を完成条件として通すことである。
...
- 旧 backend validation は破棄し、A3 の bundled kanban は SoloBoard 前提に固定する
...
- step 9: isolated validation で single / parent-child が完走した後は、generic operator default を SoloBoard に固定し、旧 backend compatibility path は削除する
...
- `task soloboard:validation` を追加し、current kanban compatibility surface を SoloBoard に対して一通り打つ parity validation を実行できる
- `task kanban:up/down/logs/url/doctor/api/bootstrap:*` は SoloBoard 専用 entrypoint とし、旧 backend env / task alias は削除する
- `task a3:portal:cutover:doctor` を追加し、SoloBoard backend で `kanban:doctor -> watch-summary -> describe-state -> kanban:validation` を一連で観測できるようにした。旧 compatibility path の比較入口は削除する
- historical isolated validation 用に `task a3:portal-soloboard:scheduler:run-once`, `task a3:portal-soloboard:watch-summary`, `task a3:portal-soloboard:describe-state`, `task a3:portal-soloboard:scheduler:control` を保持しているが、2026-04-13 の release boundary 整理で maintenance-only internal task とし、通常 operator surface からは外した
- historical evidence として、retired `task a3:portal-soloboard:direct-validation:run-once` で isolated single full-phase validation を流し、`Portal#17` が `implementation -> verification -> merge -> Done` まで完走した
- historical evidence として、retired `task a3:portal-soloboard:parent-child:direct-validation:run-once` と isolated `watch-summary` / `describe-state` で `Portal#18/#19/#20` が `Done` まで完走した。direct validation run entrypoint は後続整理で削除済みである
- isolated validation の no-op 実行では `executed 0 task(s); idle=true stop_reason=idle` を確認し、さらに `plan-next-runnable-task` で SoloBoard 上の labeled `To do` task が `next runnable ... at implementation` と選抜されるところまで確認した
...
- `task kanban:validation` も `KANBAN_BACKEND=soloboard` で generic 導線から実行できる
- `Portal`, `OIDC`, `A3Engine` の board / lane / tag bootstrap は generic `kanban:bootstrap:*` からも実行できる

2026-04-12 の parity 再確認では、standalone SoloBoard (`http://localhost:3460`) に対して `task soloboard:doctor`, `task soloboard:bootstrap`, `task soloboard:validation` を実行し、board / lane / tag / ticket / relation / comment / transition surface が維持されていることを確認した。`soloboard:validation` が作成した `Portal#5/#6` は確認後に `Done` へ transition 済みである。

したがって、step 1 から step 5b と isolated validation の single / parent-child 完走までは確認済みであり、残る main work は「未使用 command contract の parity 確認」「長期 scheduler-loop 運用と read-after-write hardening」「Docker/runtime packaging」「実 Portal source の `repo:both` parent/full verification validation」である。旧 backend compatibility path は削除対象であり、実行入口としては維持しない。
...
- bundle contents
  ...
  - project runtime container or host runtime with public `a2o-agent`
...
- cutover-time operator entrypoints to preserve (historical; not current A2O public surface)
  - `task kanban:*`
  - `task a3:portal:*`
  - `task a3:portal:cutover:doctor`
- removed historical compatibility entrypoints before freeze
  - 旧 backend env を指定する `task kanban:*`
  - 旧 backend 専用 task alias
- freeze acceptance before compose lock
  - isolated single validation が `Done`
  - isolated parent-child validation が `Done`
  - current mainline cutover doctor が SoloBoard default で通る
  - 実 Portal source の `repo:both` parent/full verification validation が通る
  - repeated scheduler-loop 観測で追加 hardening 要否が判断できる
  - historical local compose bundle (`task a3:portal:bundle:up/bootstrap/doctor/validation`) が通る

2026-04-11 時点では、workspace root に `docker-compose.a3-portal-soloboard.yml` と `docker/a3-runtime/Dockerfile` を置き、`task a3:portal:bundle:up`, `:doctor`, `:bootstrap`, `:validation`, `:down`, `:logs` を追加した。2026-04-13 の配布整理で compose 正本は `a3-engine/docker/compose/a3-portal-soloboard.yml`、runtime Dockerfile 正本は `a3-engine/docker/a3-runtime/Dockerfile` へ移動した。local bundle は `a3-runtime` container と `soloboard` container を同一 compose project で起動し、`doctor` で `ruby`, transitional `python3`, `task`, SoloBoard `/api/boards` を確認し、`bootstrap` で `Portal` / `OIDC` / `A3Engine` board/lane/tag surface を seed し、`validation` で relation/comment/transition を含む compatibility surface を実機確認できる。`python3` は runtime kanban adapter が `tools/kanban/cli.py` を subprocess 実行する間だけ残す。

ただし、この spike では Portal verification を A3 runtime container 内で直接実行するために Temurin 25 JDK を A3 image へ入れており、これは完成形ではない。完成形では A3 container は汎用 orchestration/control plane とし、project 固有 toolchain は host または project dev-env container に配置した public `a2o-agent` が引き受ける。したがって次の slice は「A3 image から project runtime を剥がす」「A3 job protocol と agent MVP を設計する」「SoloBoard 固定 compose bundle に public `a2o-agent` worker を接続する」に寄せる。

#### 0.4.5.1b A3 container / SoloBoard / project agent deployment shape

A3 の Docker 配布は single container ではなく、SoloBoard と project runtime agent を含む compose bundle として設計する。SoloBoard は A3 image に内包しない。A3 は SoloBoard API を bundled kanban として利用し、project command 実行は public `a2o-agent` へ job として委譲する。
...
- `a2o-agent` (internal compatibility name: `a3-agent`)
...
`a2o-agent` の技術選択は macOS / Linux / WSL2 Ubuntu 向け single binary を優先し、Go を正本とする。理由は process execution、signal/cancel、file/HTTP transport、JSON を runtime 依存なしで持てるためである。Windows native は標準対象外とし、Windows 利用時は WSL2 Ubuntu 上で Linux runtime として扱う。A3 本体が Ruby であっても、A3 と agent の境界は JSON job protocol にするため言語差は domain model へ漏れない。`a3-agent` は内部互換名としてだけ残す。
...
- host runtime
  - host に install した `a2o-agent` が local `task` / `mvn` / `java` を実行する
- docker dev-env runtime
  - project dev-env image に install した `a2o-agent` が container 内の toolchain で実行する

いずれの場合も、A3 は SoloBoard と job result だけを見て phase を進める。project 固有 runtime を A3 image へ bake しないことを配布設計の完了条件に追加する。

workspace materialization の owner は `a2o-agent` 側 runtime とする。A3 は source descriptor / repo slot / target ref / isolation requirement を job に含めるが、checkout/worktree 作成、実行前 dirty check、実行後 cleanup、quarantine snapshot 作成は agent が配置された runtime の filesystem で行う。A3 は agent が返す `workspace_descriptor` と artifact/evidence を検証し、phase 開始前の existence guarantee を domain rule として判定する。これにより host runtime と docker dev-env runtime の両方で同じ protocol を使える。

A3 Engine は control plane であり、Docker + host/dev-env agent mode では project repo の filesystem mutation owner ではない。Engine が行ってよいのは `workspace_request` / merge-publish request の作成、agent result の検証、artifact/evidence の記録、task/run state の更新である。project repo に対する worktree 作成、branch checkout、commit、merge、cleanup、quarantine は `a2o-agent` が Engine から受け取った agent environment config の `source_paths` と `workspace_root` に基づいて実行する。これらの path は agent 視点の値であり、Engine container path として扱わない。Ruby Engine 側の direct publication / merge implementation は削除済みであり、未設定時は `DisabledWorkspaceChangePublisher` / `DisabledMergeRunner` が fail closed する。`LocalGitWorkspaceBackend` は workspace preparation / cleanup の transitional surface として残るが、project repo mutation の正本にはしない。

`start-run` は run state を開始するだけで project workspace を materialize しない。worker / verification / merge の agent-owned path では phase execution でも Engine workspace preparation を skip し、control-plane trace 用の synthetic workspace だけを使う。project repo の実 worktree は agent job request (`workspace_request` / `merge_request`) を受けた `a2o-agent` が作成する。

2026-04-11 時点では、Ruby domain 側の最小 contract として `AgentJobRequest`、`AgentJobResult`、`AgentArtifactUpload`、`AgentWorkspaceDescriptor`、`AgentWorkspaceRequest` を追加した。これは agent 本体の完成実装ではなく、A3 control plane が受け入れる job/result JSON shape を先に固定するための実装である。特に `AgentJobResult` は `stdout_log` / `stderr_log` / `combined_log` / `artifacts` の local path field を拒否し、A3-managed artifact store の upload reference だけを受け付ける。あわせて JSON-backed job store と `/v1/agent/jobs` / `/v1/agent/jobs/next` / `/v1/agent/jobs/{job_id}` / `/v1/agent/jobs/{job_id}/result` の pull handler を追加し、`a3 agent-server` から同 API を実 HTTP endpoint として listen できる最小 entrypoint まで追加済みである。この endpoint は local host / compose network 内の control-plane API であり、central server API ではない。さらに file-backed artifact store と `PUT /v1/agent/artifacts/{artifact_id}` を追加し、digest / byte size を A3 側で検証してから upload metadata を保存できるようにした。Go toolchain がない環境でも protocol を検証できるよう、Ruby reference agent class を追加し、1 job poll、local command 実行、combined log / artifact upload、result submit までの挙動を spec fixture として固定した。これは配布 executable ではない。現在は `agent-go` module を追加し、同じ 1 job worker loop を Go standard library のみで build / test できる状態にした。2026-04-14 に `a3 agent package list/export/verify` と runtime image への `/opt/a3/agents` 同梱を実装し、host/project-dev-env へ checksum verified `a2o-agent` binary を取り出す導線を固定した。historical spike では agent-local profile file が repo alias / local path と doctor 入力を持っていたが、完成形では agent environment config を Engine 側 project config に集約し、job payload と doctor job として agent へ渡す。agent 起動引数は Engine/control-plane URL と token に最小化する。`a3 agent-artifact-cleanup` で diagnostic/evidence artifact の retention/GC を operator command として実装済みである。標準 lifecycle は `a2o host install` / `a2o project bootstrap` / `a2o kanban ...` / `a2o agent install` に寄せ、direct `a2o-agent --loop --poll-interval ...` は dev-env residency、diagnostic、operator-controlled fallback として扱う。OS service template / install / load / enable は不要と判断し、A3 distribution から外した。auth は token file を優先する local shared bearer token を最小 contract とする。残りは Engine-managed agent environment config、runtime loop process、agent-owned publish/merge、disk/artifact retention、長時間運用 hardening である。remote TLS / remote agent authorization scope / multi-node scheduling は現行完成条件に含めない。

補足: agent auth の現行 contract は token file と scope 分離を含む。agent 側 endpoint は `A3_AGENT_TOKEN_FILE` / `--agent-token-file` / profile `agent_token_file`、A3 control 側 enqueue/fetch endpoint は `A3_AGENT_CONTROL_TOKEN_FILE` / `--agent-control-token-file` を優先し、process arguments へ token を露出させない。control token 未設定時は local/backward-compatible path として agent token に fallback する。長時間起動する `a2o-agent` と `a3 agent-server` は request ごとに token file を読み直すため、token file の atomic replace で restart なしの rotation が可能。

#### 0.4.5.2 phase model 再検討メモ

旧 backend baseline validation を通した結果、A3-v2 のような自動実行では `review` を独立 phase として持つ価値が薄く見えていた。特に軽量 task では、`review` を通しても最終的には `verification` の通過可否で戻し先が決まるため、中間 phase と Kanban 列の往復がオーバーヘッドになりやすい。2026-04-10 時点では、この再設計は fresh `single` / `child` について実装済みで、現行正本は `single/child = implementation -> verification -> merge`, `parent = review -> verification -> merge` である。
...
- 検討の順番
  - まず既存の SoloBoard baseline exit rule と実 Portal source の `repo:both` parent/full verification validation を満たす
  - その後に `implementation + review` 統合案を別 task 化して設計する
  - `review + verification` 統合は副案として比較する

#### 0.4.5.3 phase redesign の次スライス

2026-04-10 時点では cleanup / retention の基盤と parent-child validation が通ったため、phase 再編は「次に着手してよい設計課題」に上がった。ただし current 正本を直接崩す段階ではなく、まず single / child を対象に移行 slice を固定する。
...
- slice 1 の acceptance
  - single validation が `implementation -> verification -> merge` で完走する
  - child validation が `implementation -> verification -> merge` で完走する
  - parent-child validation で、child 完了後に parent だけが `review -> verification -> merge` へ handoff される
...
5. `Portal#131`-`Portal#143` の scheduler validation 前に、runtime validation で command template 展開と result JSON 回収を確認する
...
- test fixture dependency:
  ...
  - stale な deleted legacy root script 参照 spec は削除済み。stdin bundle worker / direct validation worker の current spec は engine library を直接対象にする
...
- parity backlog の項目は「後で改善」ではなく、Portal live validation の blocker として扱う
- v1 で一度解決した性質は、v2 で未実装なら bug と同列に扱う
- 以後の scheduler / merge / cleanup 修正は、この監査表のどの項目を回収するものかを明示して進める

### 0.5 次の Canaries

次の live validation は、今回の `repo:starters` 単独 thin handoff validation を踏まえて、次のどちらかを小さく選ぶ。

- cross-repo child を含む parent finalize validation
  - parent merge inventory / child commit 集約 / live promotion の再確認
- `repo:ui-app` 単独 validation
  - ui-app 側の canonical flow と support repo requirement を分離して確認

優先は前者である。今回の live handoff validation で見えた gap は merge / parent inventory 寄りだったため、次は parent finalize の live validation を薄く回す価値が高い。

## 1. 目的

- A3 を共通 Docker image として配布可能にする
- bundled kanban は SoloBoard とし、compose service として A3 と同梱できるようにする
- 案件ごとの差分を A3 image rebuild ではなく runtime package と `a2o-agent` 配置先 runtime で扱えるようにする
...
つまり、案件差分は A3 image に焼き込まず、runtime package、環境変数/secret/volume、`a2o-agent` の runtime profile で与える。
...
- doctor / inspect / runtime package inspection は、project runtime の薄い実地試験に使える `runtime_validation_command` を返し、doctor -> migration -> runtime の順をそのまま辿れるようにする
...
### 4.3 Engine-managed agent environment config の責務

^`a2o-agent` は runtime package の内容を解釈して A3 domain rule を実装するものではなく、A3 が発行した job を配置先 runtime で安全に実行する worker である。

agent environment config は Engine 側 project config として保管し、少なくとも次を表す。

- agent name
- workspace root
- source paths (`source alias -> agent-visible local git path`)
- allowed commands
...
agent environment config が持たないもの:
...
runtime package と agent environment config の責務は分ける。

- runtime package: A3 が job payload に使う repo slot、workspace freshness / cleanup policy、worker gateway option summary を公開する。
- agent environment config: host/dev-env 側で `source alias -> local git path` と workspace root を保持する。値は agent 視点であり、Engine container path ではない。
- A3 doctor: config schema、必須 slot coverage、workspace policy 値を検査する。agent runtime filesystem には触らない。
- Engine-issued agent doctor job: local git path の存在、git worktree 性、dirty 状態、workspace root の書き込み可否を agent 側で検査する。
- `a2o-agent` launch args: Engine/control-plane URL と token のみに最小化する。repo path や workspace root は agent の local profile file ではなく Engine-provided job/doctor payload で受け取る。

2026-04-14 実装状態: `AgentJobRequest` / Go `JobRequest` に optional `agent_environment` を追加し、`workspace_root`、`source_paths`、`env`、`required_bins` を job payload として渡せるようにした。`AgentWorkerGateway`、`AgentCommandRunner`、`AgentMergeRunner` は同 payload を enqueue し、Go agent は `agent_environment.workspace_root` と `agent_environment.source_paths` がある場合、それを優先して workspace materialization / merge / cleanup に使う。`a2o-agent doctor` は `--workspace-root`、`--source-path ALIAS=PATH`、`--required-bin VALUE` を受け付け、agent-local profile file なしで Engine-managed agent environment 相当を検査できる。`a3-agent doctor` は内部互換 fallback として扱う。agent 側 local profile file は後方互換の fallback として残るが、標準導線は Engine-managed job payload / doctor flags へ寄せる。direct `a2o-agent --loop --poll-interval ...` の起動/停止手順は dev-env residency / diagnostic / operator-controlled fallback 用として扱う。
...
path 名は実装で調整しうるが、責務分離は固定する。project workspace は `a2o-agent` が配置された runtime 側にあり、A3 container の必須 writable mount ではない。A3 container が保持するのは workspace 実体ではなく、workspace descriptor、job result、artifact/evidence である。
...
- runtime を停止する場合は root で retired Portal runtime task (down) を使う
- `down` は container を止めるための操作であり、SoloBoard / A3 runtime の volume を消す操作としては扱わない
- 再度検証する場合は retired Portal runtime task (up), retired Portal runtime task (doctor), retired Portal runtime task (bootstrap) の順に戻す
- runtime validation storage の古い blocked が観測を汚す場合は、削除ではなく retired Portal runtime task (archive-state) で退避してから観測を再開する
- 状態を一定間隔で残す場合は `ITERATIONS=3 INTERVAL_SECONDS=30 <retired Portal runtime observe task>` を使う

artifact store の掃除:

- まず retired Portal runtime task (agent-artifact-cleanup) dry-run で削除候補を確認する
...
- 2026-04-12 の retired Portal runtime task (agent-artifact-cleanup) dry-run では `deleted_count=0`, `retained_count=0`, `missing_blob_count=0` を確認した
- 2026-04-12 の post-archive recovery validation では retired Portal runtime task (doctor), retired Portal runtime task (bootstrap), retired Portal runtime task (validation), retired Portal runtime task (agent-artifact-cleanup) dry-run が成功した。`runtime:validation` が作成した `Portal#93/#94` は確認後に `Done` へ transition 済みである
- 同日の runtime agent validation sweep 後にも retired Portal runtime task (agent-artifact-cleanup) dry-run は `deleted_count=0`, `retained_count=0`, `missing_blob_count=0` を返し、artifact cleanup 対象が残っていないことを確認した
...
- archived state `/var/lib/a3/archive/portal-soloboard-bundle-validation-20260411T142541Z` には `Portal#43` の blocked diagnosis が保持されている
...
- CI/long-running observation ではなく local validation が完了しただけなら、container は止めてよい
- disk pressure が主因なら、まず artifact cleanup dry-run と Docker build cache prune を優先し、volume 削除は最後の手段にする
- 2026-04-12 の `ITERATIONS=1 INTERVAL_SECONDS=1 <retired Portal runtime observe task>` では host disk 空き約 88GiB、Docker reclaimable 約 3.7GiB、runtime state は `active_runs=0`, `queued_tasks=0`, `blocked_tasks=0` だった
- 同日の `ITERATIONS=3 INTERVAL_SECONDS=30 <retired Portal runtime observe task>` でも全 iteration で runtime doctor / watch-summary / describe-state が成功し、host disk 空き約 89GiB、Docker reclaimable 約 3.7GiB、`active_runs=0`, `queued_tasks=0`, `blocked_tasks=0` を維持した
- 同日の retired Portal runtime task (archive-state) では active storage を `/var/lib/a3/archive/portal-soloboard-bundle-validation-20260412T023635Z` へ退避し、直後の retired Portal runtime task (describe-state) / retired Portal runtime task (watch-summary) で新しい active storage が `active_runs=0`, `queued_tasks=0`, `blocked_tasks=0` の idle として読めることを確認した
- 同日の runtime agent validation sweep 後の `ITERATIONS=1 INTERVAL_SECONDS=1 <retired Portal runtime observe task>` でも runtime doctor / watch-summary / describe-state が成功し、host disk 空き約 88GiB、Docker reclaimable 約 3.7GiB、`active_runs=0`, `queued_tasks=0`, `blocked_tasks=0` を確認した
- 同日の diagnostic surface 確認後に `ITERATIONS=4 INTERVAL_SECONDS=45 <retired Portal runtime observe task>` を実行し、全 iteration で runtime doctor / watch-summary / describe-state が成功した。host disk 空きは約 89GiB、Docker reclaimable は約 3.7GiB のまま維持され、各 iteration の state は `active_runs=0`, `queued_tasks=0`, `blocked_tasks=0` だった
...
explicit map で供給された repo source は、agent doctor / inspect で writable であることも確認できるべきである。A3 doctor は source descriptor と agent environment config の slot coverage を確認し、実 filesystem の writable / dirty / checkout 状態は agent doctor job が確認する。
...
phase ごとの具体的な `SourceDescriptor` や `ReviewTarget` は task/run evidence から解決されるが、A3 runtime はそれを job として表現できる repository metadata と branch/integration 解決手段を持たなければならない。実 filesystem への materialize は Engine-managed agent environment config と agent policy に従って、agent が実施する。
...
- job control plane
  - `a2o-agent` が job を pull/claim し、result を返す
- doctor / inspect
  - state / config / mount / secret の健全性確認を行う

この違いは entrypoint / command の違いであり、domain model の違いではない。

project command execution は A3 container の execution mode ではない。host runtime または project dev-env container に配置した public `a2o-agent` の責務である。
...
- `a2o-agent` job protocol endpoint
- package registry

案件 verification に必要な service へ直接通信するのは、原則として `a2o-agent` が配置された project runtime 側の責務である。A3 container が project verification のために DB / browser / test service へ直接接続する前提は採らない。
...
- project command 実行は host または project dev-env container の public `a2o-agent` が担当する
...
- Engine-managed runtime loop と worker `a2o-agent` loop の起動/停止/異常終了時の operator 手順
- file exchange transport を HTTP pull transport の後に追加するかの判断
- host runtime と project dev-env container runtime の agent environment config schema / doctor surface の hardening
- local operator 向け `docker compose` テンプレートの提供範囲
- secret store 連携の標準実装

### 14.1 2026-04-11 full RSpec で観測した別領域 failures

agent environment config contract slice の focused verification は成功したが、`bundle exec rspec` 全体では次の既存別領域 failures を観測した。これらは agent environment / materialized gateway の変更範囲ではなく、別途棚卸しして扱う。
...
次に全体 green を狙う場合は、上記を production bug と spec drift に分ける。現時点の優先は fixture drift の棚卸しであり、agent materialized runtime profile の focused path は `go test ./...`、runtime package / doctor focused specs、materialized validations で確認済みである。
...
- project command execution を public `a2o-agent` に委譲し、A3 image に project toolchain を bake しない方針が定義されている
...
- OS service 化を標準導線にせず、`a2o project bootstrap` / `a2o kanban ...` / `a2o agent ...` を標準導線とし、必要時の `a2o-agent --loop --poll-interval ...` は dev-env residency / diagnostic / operator-controlled fallback として分離している
