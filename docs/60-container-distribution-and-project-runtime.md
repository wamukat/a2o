...
- 完了済み
  ...
  - local release image equivalent `a3-portal-bundle-a3-runtime` から `a2o host install` し、bootstrap 後に package 指定なしで `a2o runtime up` / `a2o agent install` / `a2o runtime doctor` / `a2o runtime command-plan` が動くことを smoke 確認済み。public launcher `a2o`、compat alias `a3`、public share dir `share/a2o`、public agent path `.work/a2o-agent/bin/a2o-agent` を確認した。smoke 用 compose project `a2o-release-smoke` と `.work/a2o-release-smoke` は停止・削除済み
...
- 未完了
  ...
  - 正式 registry image ref 公開後に、`a2o host install` / `a2o project bootstrap` / `a2o runtime up` / `a2o agent install` / `a2o runtime doctor` / `a2o runtime command-plan` を registry image で再 smoke すること
...
- runtime run loop の通常入口は A2O public lifecycle command と reference product package で提供する。Portal runtime task は historical / maintenance-only surface とし、root-local `pause-scheduler` / `resume-scheduler` / `show-scheduler-state` task は maintenance-only internal surface として通常 operator runbook から外す。runtime 標準は operator terminal での one-shot / manual loop 起動であり、OS service 化は current scope 外である。macOS LaunchAgent 用の `a3:portal:scheduler:install/uninstall/reload/status` と `scripts/a3/launchd.rb` は削除済みで、`a2o-agent` の scheduler 登録機能としては扱わない
