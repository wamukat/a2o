...
- 未実装 / 未完
  - `a3-agent` の manual loop 運用と長時間運用 hardening。`a3-agent --loop --poll-interval ...` は実装済みで、OS service template / install / load / enable は標準導線から外す。auth も local token file の最小 contract は実装済みで、残りは local-first 配布手順と長時間運用 hardening である。中央 A3 server / remote multi-agent pool / remote TLS termination は現行スコープ外とする
