...
- 未実装 / 未完
  - `a3-agent` の local daemon 管理 / 長時間運用 hardening。service 化の入口として loop mode、service-manager template rendering、local installer からの opt-in template install と load/enable hook は実装済み。auth も local token file の最小 contract は実装済みで、残りは local-first 配布手順と長時間運用 hardening である。中央 A3 server / remote multi-agent pool / remote TLS termination は現行スコープ外とする
...
- `a3-agent` の Go single binary service template を local 配布 package で install/load/enable する手順と daemon 管理
