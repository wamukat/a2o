...
- 未実装 / 未完
  ...
  - 実 Portal source の `repo:both` parent/full verification canary を A3 完成条件に含める
...
- 次は `SoloBoard 前提の配布導線`、`manual loop / retention hardening`、実 Portal source の `repo:both` parent/full verification canary を進める。Kanboard compatibility path は current runtime から削除する
