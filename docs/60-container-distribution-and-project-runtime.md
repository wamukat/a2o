...
- 完了済み
  ...
  - A3 runtime run-once surface (`task a3:portal:runtime:run-once`) を workspace root から起動できる。historical scheduler control は maintenance-only とし、macOS LaunchAgent service surface は current scope 外として削除済み
  ...
  - `task a3:portal:runtime:agent-real-parent-full-verification-validation` で実 Portal live repo の `repo:both` parent/full verification validation を完走した。`Portal#88` parent と `Portal#89/#90` child を作成し、実 `member-portal-starters` / `member-portal-ui-app` live repo に parent validation commit を作成した上で、starters 側 `gate:standard` / `test:all` / `test:nullaway` と ui-app 側 `gate:standard` / `test:nullaway` を agent-http 経由で実行し、parent merge が両 repo の `feature/prototype` を `refs/heads/a3/parent/Portal-88` へ更新することまで確認した
- 未完了
...
- `task a3:portal:runtime:agent-verification-validation`
...
- `task a3:portal:runtime:agent-parent-topology-validation`
...
- `task a3:portal:runtime:agent-real-parent-full-verification-validation`
...
5. runtime loop 起動
   - current implementation では `task a3:portal:runtime:run-once` が 1 cycle 実行であり、Engine-managed continuous loop は次スライスで追加する
   - 完成形では `task a3:portal:runtime:loop` が runtime container 内で interval 実行し、operator は `watch-summary` / `describe-state` / `pause` / `resume` で状態を見る
6. 反復観測
...
- `T1 host-local agent protocol`: `task a3:portal:runtime:host-agent-validation` で Docker 上 A3 control plane と host local `a3-agent` の pull / command execution / artifact upload / result submit を確認する
...
- `T3 host-local parent topology`: parent / child relation、repo:both slot、parent verification、merge が host local `a3-agent` 経由で `Done` まで進むことを `task a3:portal:runtime:host-agent-parent-topology-validation` で確認する
...
- 停止は `task a3:portal:runtime:down` または loop process の graceful stop を標準とする。強制 kill 後は `task a3:portal:runtime:describe-state` と `task a3:portal:runtime:watch-summary` で active / queued / blocked を確認する
...
- SoloBoard bootstrap / doctor / validation: `task soloboard:doctor`, `task soloboard:bootstrap`, `task soloboard:validation`, generic `task kanban:*`
...
- agent 正規経路 validation: `task a3:portal:runtime:host-agent-validation`, `:host-agent-parent-topology-validation`, `:run-once`
...
- runtime validation storage の古い blocked が観測を汚す場合は、削除ではなく `task a3:portal:runtime:archive-state` で退避してから観測を再開する
...
- 2026-04-12 の post-archive recovery validation では `task a3:portal:runtime:doctor`, `task a3:portal:runtime:bootstrap`, `task a3:portal:runtime:validation`, `task a3:portal:runtime:agent-artifact-cleanup -- --dry-run` が成功した。`runtime:validation` が作成した `Portal#93/#94` は確認後に `Done` へ transition 済みである
- 同日の runtime agent validation sweep 後にも `task a3:portal:runtime:agent-artifact-cleanup -- --dry-run` は `deleted_count=0`, `retained_count=0`, `missing_blob_count=0` を返し、artifact cleanup 対象が残っていないことを確認した
...
- 同日の `task a3:portal:runtime:archive-state` では active storage を `/var/lib/a3/archive/portal-soloboard-bundle-validation-20260412T023635Z` へ退避し、直後の `task a3:portal:runtime:describe-state` / `task a3:portal:runtime:watch-summary` で新しい active storage が `active_runs=0`, `queued_tasks=0`, `blocked_tasks=0` の idle として読めることを確認した
- 同日の runtime agent validation sweep 後の `ITERATIONS=1 INTERVAL_SECONDS=1 task a3:portal:runtime:observe` でも runtime doctor / watch-summary / describe-state が成功し、host disk 空き約 88GiB、Docker reclaimable 約 3.7GiB、`active_runs=0`, `queued_tasks=0`, `blocked_tasks=0` を確認した
