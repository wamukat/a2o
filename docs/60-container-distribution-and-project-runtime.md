...
標準運用は Engine-managed runtime loop とする。operator が OS service を登録するのではなく、A3 runtime container 内の 1 process が `execute-until-idle` 相当の cycle を interval 実行し、agent job queue と runtime state を管理する。`a3-agent` は scheduler を持たず、control plane を poll して job を実行する worker に徹する。これにより `systemd` / `launchd` / Windows Service の差分を A3 配布物から外しつつ、kanban 自動実行の所有者を A3 Engine へ戻せる。
...
- `a3-agent`: host または project dev-env で job を pull し、git worktree / branch / project command / merge / artifact upload を実行する。
- OS / shell: compose 起動と agent process 起動だけを担当する。service manager integration は A3 の標準配布範囲外とし、必要な利用者が外側で wrapper 化する。

配布物:

- `a3-engine` image: `/usr/local/bin/a3`, runtime loop entrypoint, agent control-plane API, SoloBoard adapter, storage/retention tooling, multi-platform `a3-agent` release binaries を含む。
- `a3-agent` export package: Engine image に同梱した Go single binary を `a3 agent package list/export/verify` 相当の command で host / project dev-env へ取り出す。macOS / Linux / WSL2 Ubuntu を対象とし、Windows native は対象外。
...
- Engine image は release unit の正本として、Engine と同じ protocol version の `a3-agent` binaries を `/opt/a3/agents/<target>/a3-agent` のような場所に同梱する。
...
- export は manifest の SHA-256 を検証してから archive 内の `a3-agent` binary を指定先へ展開し、利用者 host に Go toolchain / Ruby interpreter を要求しない。

想定 command:

```bash
# 同梱 agent binary 一覧
docker run --rm a3-engine:latest a3 agent package list
...
docker run --rm \
  -v "$PWD/.a3/bin:/out" \
  a3-engine:latest \
  a3 agent package export --target linux-amd64 --output /out/a3-agent
```
...
3. agent 設置
   - host 実行では Engine image から `a3-agent` binary を export して install する
   ...
   - 標準起動は `a3-agent --engine <control-plane-url> --token-file <path> --loop --poll-interval 2s` を operator terminal または dev-env container で実行する
...
4. agent 疎通確認
   - `a3-agent doctor --engine <control-plane-url> --token-file <path>`
...
- freeze acceptance before compose lock
  ...
  - local compose bundle (`task a3:portal:bundle:up/bootstrap/doctor/validation`) が通る
...
`a3-agent` の技術選択は macOS / Linux / WSL2 Ubuntu 向け single binary を優先し、Go を正本とする。理由は process execution、signal/cancel、file/HTTP transport、JSON を runtime 依存なしで持てるためである。Windows native は標準対象外とし、Windows 利用時は WSL2 Ubuntu 上で Linux runtime として扱う。A3 本体が Ruby であっても、A3 と agent の境界は JSON job protocol にするため言語差は domain model へ漏れない。
...
- Engine-managed runtime loop と worker `a3-agent` loop の起動/停止/異常終了時の operator 手順
