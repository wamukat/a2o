...
- agent name
- workspace root
- source paths (`repo slot -> agent-visible local path`)
- allowed commands
...
- agent environment config: host/dev-env 側で `repo slot -> local git path` と workspace root を保持する。値は agent 視点であり、Engine container path ではない。
