...
- TODO-4: branch-native publish / merge
  ...
  - 2026-04-12 時点で implementation publish の Go agent path は実装済み。残りは merge job を同じ agent-owned branch worktree contract に移すことと、移行完了後に Ruby `LocalWorkspaceChangePublisher` / `LocalMergeRunner` の direct/local compatibility を主経路から削除または明確に隔離すること
