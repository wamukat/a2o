...
- TODO-2: agent workspace request full slots
  ...
  - support slot は task 専用 ref (`refs/heads/a3/work/*` / `refs/heads/a3/parent/*`) を使わず、project live/base ref を read-only で materialize する。task/parent ref は edit-target slot 専用の名前空間とし、support repo に同名 ref を作らない
