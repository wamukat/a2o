...
- 完了済み
  ...
  - local release image equivalent `a3-portal-bundle-a3-runtime` から `a3 host install` し、bootstrap 後に package 指定なしで `a3 runtime up` / `a3 agent install` / `a3 runtime doctor` / `a3 runtime command-plan` が動くことを smoke 確認済み。smoke 用 compose project `a3-release-smoke` と `.work/a3-release-smoke` は停止・削除済み
...
- 未完了
  ...
  - Docker Hub / GHCR の正式 image ref を決定し、利用者 quickstart の placeholder を置換すること
  - 正式 registry image ref 公開後に、`a3 host install` / `a3 project bootstrap` / `a3 runtime up` / `a3 agent install` / `a3 runtime doctor` / `a3 runtime command-plan` を registry image で再 smoke すること
