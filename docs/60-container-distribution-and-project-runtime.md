...
- TODO-4a: AI-assisted merge conflict recovery
  ...
  - implementation slices:
    ...
    - Slice 5: partially done. recovery publish 後に task-scoped `verification_source_ref` を設定し、次の verification run が published target ref を検証するようにした。`retryable` / `terminal_noop` の verification rerun でも同 ref を保持する。残りは watch-summary / SoloBoard comment / `show-run` の `MergeRecovery` evidence 表示を operator surface として整える
