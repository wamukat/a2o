...
この validation 群は `docker:a3 + docker:soloboard + docker:dev-env(a3-agent)` 形状が成立していた historical evidence である。現行配布形状は `docker:a3 + docker:soloboard + host/project-dev-env a3-agent binary` であり、Docker dev-env agent image は配布対象外である。
...
注意: legacy direct-path diagnosis 用の旧 direct run-once task は削除済みである。A3 runtime container には project 固有 Java/Maven runtime を持たせない方針のため、Docker A3 から Portal verification を直接実行する経路は通常 surface に戻さない。worker/verification/parent topology の検証は host-local `a3-agent` 経路を正規経路とする。
...
2026-04-13 時点の配布 runbook は、`docker:a3 + docker:soloboard + host/project-dev-env a3-agent binary` を標準形状として扱う。A3 container は汎用 control plane と runtime loop の所有者であり、Portal 専用 JDK / Maven / Node runtime を持たない。project command は host または project dev-env container に install した `a3-agent` が担当する。A3 release に含める Dockerfile は `a3-engine/docker/a3-runtime/Dockerfile` を正本とし、A3 Engine command は image 内の `/usr/local/bin/a3` として提供する。host に Ruby interpreter や host Ruby `a3` executable を install させない。workspace root 直下の `docker/a3-agent` / `docker/a3-portal-agent` image は削除済みである。
