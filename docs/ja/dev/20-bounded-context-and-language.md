...
この文書は、A2O の用語と bounded context を定義する。
core domain model、workspace model、evidence model、Ruby 実装は、この文書の語彙と境界を前提にする。

## 1. 目的

- 同じ概念を別の場所で別名・別意味にしない
- domain knowledge を一貫した語彙と責務境界へ寄せる
- `review`, `verification`, `workspace`, `evidence` などの意味を先に確定し、後続の設計と実装のぶれを防ぐ

## 2. 設計姿勢

A2O では、用語を「コードの都合」ではなく「ドメインの意味」で定義する。

- 先に意味を固定し、その後で class/module/file 名を合わせる
- phase ごとの便宜的な救済分岐を domain language に持ち込まない
- A2O の主要語彙は、この文書に定義した phase と context に揃える

## 3. Context Map

現時点では、A2O の主要 context を次の 4 つに分ける。
...
A2O が管理する実行単位。kanban task に対応するが、単なる外部 ID ではなく、kind・status・phase history を持つ domain object として扱う。
...
`inspection` は主要語彙に採用しない。必要な判断は `review`、`verification`、`merge` のいずれかに位置づける。
