...
`state` を省略した feedback は `new` として扱う。採用判断が進んだら `accepted`、`rejected`、`converted_to_ticket`、`applied` のいずれかに分類する。`describe-task` は pending feedback を表示し、横断確認では `a2o runtime skill-feedback list --state new --group` で重複候補をまとめて確認できる。

自動書き換えは行わない。候補は `a2o runtime skill-feedback propose --format ticket` でチケット本文の draft に変換でき、`--format patch` ではレビュー前提の draft patch を出力する。永続ログ cleanup は `combined-log`、`ai-raw-log`、`execution-metadata` を対象にできるが、証跡に保存された `skill_feedback` の要約は実行診断の一部として残す。
