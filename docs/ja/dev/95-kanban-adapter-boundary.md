...
読む目的は、A2O のドメイン状態と Kanbalone の API / CLI 仕様を直接混ぜないための境界を理解することである。Kanbalone は SoloBoard からリネームされた後継である。公開アダプター backend 名は `kanbalone` とし、`soloboard` は古い project / runtime wiring のための互換 alias としてだけ残す。カンバンは利用者に見えるタスクキューだが、Engine 内部では操作単位のクライアントを通して読み書きし、レーン名や解決済みフラグの意味を明示的に変換する。
...
新しいアダプター設定では `KANBALONE_BASE_URL`、`KANBALONE_API_TOKEN`、`KANBAN_BACKEND=kanbalone` を使う。`SOLOBOARD_BASE_URL`、`SOLOBOARD_API_TOKEN`、`KANBAN_BACKEND=soloboard` は互換 fallback としてだけ扱う。Compose service 名や packaged compose file 名にはまだ `soloboard` が残る場合があるが、これらは runtime packaging の互換面であり、CLI backend 名とは別段階で移行する。

## ランタイムの Kanbalone モード

A2O のランタイムインスタンスは 2 つのモードを持つ。

- `bundled`: A2O がプロジェクトインスタンス用の同梱 Kanbalone サービスを起動・管理する。
- `external`: 既存の Kanbalone endpoint へ接続し、設定されたボードの初期化と確認だけを行う。

外部モードでは、利用者向けの `kanban_url` と、任意の `kanban_runtime_url` を保持する。ホスト用ランチャーは `a2o kanban url` などの運用者向けコマンドに公開 URL を使い、ランタイムコンテナ内のコマンドは Docker ネットワークから到達できる runtime URL を使う。これにより `host.docker.internal` や Compose service 名を指定できる。アダプター契約自体は上記の subprocess CLI 契約のままで、endpoint とライフサイクルの所有者だけが変わる。
...
A2O 0.5.36 は `docker/a3-runtime/Dockerfile` に `python3` を残すが、`python3-venv` はインストールしない。
