...
MVP では、`trigger:investigate` による調査、proposal 作成、proposal review を通常の実装スケジューラから独立して実行できることを確認した。A2O#356 では、clean な proposal review の後の運用体験を変える。A2O が早い段階で draft child を Kanban に作り、運用者が Kanban 上で編集・承認できるようにする。
...
- clean な proposal review の後、child ticket を draft として自動作成または再同期する。
...
| `a2o:decomposed` | A2O | source ticket について、現在の proposal lineage で少なくとも一度 draft child creation / reconciliation が成功している。 |
...
| `trigger:auto-parent` | 運用者または project policy | parent automation を明示的に要求する。decomposition によって暗黙には付かない。 |
...
A2O#356 では、review 済み clean proposal の後に draft creation stage を自動で追加する。
...
5. review が clean なら draft child を作成または再同期する
...
Kanban comment は短く、利用者が次に何をすべきか分かる内容にする。どの draft child を作成または再同期したか、どう承認するかを伝える。詳細 JSON は evidence に置く。
...
- clean review 後に automatic decomposition が draft child を作る
...
- parent / source evidence が作成・再同期された draft ref を記録する
...
- 7章: clean review から draft reconciliation へ進む automatic scheduler stage。
