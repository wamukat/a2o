...
`metrics trends --format json` は保存済み metrics record から計算した derived indicator を返す。
...
`metrics trends` の default は `--group-by all` である。`--group-by task` と `--group-by parent` も指定できる。task metrics record から計算できない indicator は、黙って省略せず `unsupported_indicators` に列挙する。
