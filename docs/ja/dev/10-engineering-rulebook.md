...
対象読者: A2O 実装者 / reviewer
文書種別: 実装規律

この文書は、A2O 実装時に常に守るべき開発ルールを固定する。
...
A2O は現行実装であり、旧 A2O / legacy automation 互換を最優先しない。
...
## 4.1 案件固有情報を A2O 本体へ持ち込まない

A2O は product / engine であり、個別案件の名称・前提・語彙を本体の公開 contract に含めない。

- 案件名を command 名、value object、field 名、summary 文言に入れない
- 案件固有の runtime 手順は、A2O 本体ではなく project.yaml / runtime package で表現する
- 案件固有の判断は A2O の domain rule に埋め込まず、project context として外から与える
- 案件固有の build/test/remediation command、hook path、repo topology 名、repo path 規約を、A2O 同梱 preset や product docs の正本へ埋め込まない

許されるのは次だけである。

- 実行時に project.yaml / runtime package から案件情報を注入する
- operator 向け表示で injected な案件情報を透過的に出す

禁止されるもの:

- 特定案件名を A2O 本体 command や domain contract に入れる
- 特定案件向け validation / migration / workspace rule を A2O の標準機能名として固定する
- 直近導入案件の都合を、そのまま A2O の ubiquitous language に昇格させる
- 特定案件や特定 project の toolchain knowledge を、A2O の base preset / bundled preset / CLI の既定分岐として固定する
...
- 案件固有語彙や案件前提を、A2O 本体の command / domain / contract に持ち込んでいないか
- 案件固有知識を、A2O 本体の bundled preset / docs / default command / fixed repo topology に持ち込んでいないか
