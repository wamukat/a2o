...
このスクリプトは Ruby と Go suite を並列実行し、デフォルトでは `.work/test-core/` にログを書く。ログ出力先は `A2O_TEST_LOG_DIR` で変更できる。

Ruby -> Go を順番に実行する無駄をなくすため、wall-clock time を短縮できる。assertion や package は削らない。
