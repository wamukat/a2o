...
source ticket が外部 issue から import されたものでも、A2O は source ticket を要求 artifact として残し、generated parent を同じ外部 issue として再 import しない。正規化した remote metadata は child-creation evidence の `source_remote` にだけ残す。要求 source から generated implementation parent への Kanbalone `related` relation は維持する。generated parent や draft child の本文には remote metadata をコピーしないため、worker 入力のノイズを増やさず、出自が必要な場合は relation または evidence を辿る。
