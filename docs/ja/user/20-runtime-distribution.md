...
## Current 0.5.5 Surface（現在の公開 surface）

A2O 0.5.5 は、local-first runtime image、host launcher、agent package で構成される。標準 validation surface は reference product suite である。SoloBoard pickup and transitions、agent-materialized workspaces、agent HTTP worker gateway、verification、merge、parent-child flow、watch summary、describe-task diagnostics、evidence persistence を確認する。
