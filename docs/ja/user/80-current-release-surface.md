...
A2O 0.5.85 で現在利用できる公開機能と検証範囲を示す。
...
- 同梱 / runtime Compose は `a2o-runtime` を Docker init 有効で起動する。これにより、scheduler や `docker compose exec` の繰り返しで作られた終了済み子プロセスを PID 1 が回収できる。これは [GH#100](https://github.com/wamukat/a2o/issues/100) の修正である。
...
- schema-invalid な review worker result は `blocked` として fail closed する。A2O は invalid payload から raw の `rework_required`、`review_disposition`、merge recovery 制御フラグを救済しない。
...
- metrics collection 設定: `runtime.phases.metrics.commands` は verification 成功後に実行される reporting hook であり、Kanban lane を持つ独立 scheduler phase ではない。既存の phase command surface と同じ worker request 契約を使うため、この key を正規形として維持する。
- agent-materialized execution は `project.yaml` から implementation completion hook を読み込む。runtime が生成する command line では、明示的な `--agent-implementation-completion-hook` CLI option が引き続き上書きとして機能する。
- completion hook の failure diagnostics には、hook の working directory、materialized workspace root、host `PATH`、shell path、推定 executable path、stdout、stderr、exit status が含まれる。host-agent log には `completion_hooks_start` と `completion_hooks_done` も記録されるため、コンテナ内実行の誤認と host agent 側の toolchain / PATH 問題を切り分けられる。
- runtime 運用ドキュメントでは、標準の host-local agent lifecycle を明示している。`a2o runtime resume` が host scheduler を起動し、各 scheduler cycle が `.work/a2o/agent/bin/a2o-agent` を foreground の host-local agent attempt として起動する。`a2o-agent --loop` は診断 / 手動互換モードであり、通常の host 再起動復旧経路ではない。
- scheduler の task selection は、個別の不正な Kanban ticket から分離される。`trigger:auto-implement` などで選択された implementation ticket に設定済み `repo:*` label がない場合、その ticket だけを actionable warning 付きで skip し、他の valid ticket は引き続き実行される。
- scheduler task selection は、package の selection lane が `To do` に限定されていても、すでに進行中の continuation lane を復旧対象として扱う。親タスクが `Merging` に到達済みで runtime の `tasks.json` record が欠落している場合でも、scheduler recovery から見えるため、idle し続けず merge を完了できる。これは [GH#96](https://github.com/wamukat/a2o/issues/96) の修正である。
- recoverable merge conflict は、同じ merge を `max_steps` まで繰り返さず task を blocked にする。blocked diagnosis と Kanban comment には merge recovery の conflict file list を含めるため、operator は retry 前に競合ファイルの解消または再生成を判断できる。これは [GH#97](https://github.com/wamukat/a2o/issues/97) の修正である。
- 起動直後の `HTTP 503` など、一時的な Kanbalone readiness failure は Kanban command 境界で retry され、即座に scheduler cycle をクラッシュさせない。
- runtime host-agent export では、`agent package verify` の TCP open timeout や一時的 DNS failure など transient network failure を warning-only として扱い、export を継続する。package integrity や publication の非 transient error は引き続き export 前に fail する。
- read-only observer hook: `runtime.observers` で phase / task event 向けの project-owned notification / audit command を実行できる。observer の結果は best-effort diagnostics であり、task progress、phase outcome、agent feedback、workspace output は変更しない。詳細は [90-project-package-schema.md#runtime-observers](90-project-package-schema.md#runtime-observers) を参照する。
...
- decomposition child creation の source-ticket comment には、`--all`、`--child`、`--ready` の具体的な `accept-drafts` CLI 例を表示する。project package で `kanban.system_comment_locale: ja` を設定すると、この decomposition audit comment を日本語で表示する。既定は `en` である。
...
- project package public key naming: `project.yaml` の公開設定キーは snake_case を正規形とする。`runtime.prompts.repo_slots`、`child_draft_template`、docs surface の `repo_slot` が正規キーであり、旧 `runtime.prompts.repoSlots` / `childDraftTemplate` / docs surface の `repoSlot` は移行対象である。詳細は [90-project-package-schema.md#public-key-naming-convention](90-project-package-schema.md#public-key-naming-convention) と [90-project-package-schema.md#runtime-prompts](90-project-package-schema.md#runtime-prompts) を参照する。
- 公開 hook / command モデル: user-authored execution 設定を phase command、implementation completion gate、publish preflight、read-only observer / reporting、delivery hook として整理した。汎用 mutable lifecycle hook は追加せず、変更可否と失敗時の意味を明確にする。詳細は [90-project-package-schema.md#公開-command--hook-モデル](90-project-package-schema.md#公開-command--hook-モデル) を参照する。
- docs surface の repository slot key は snake_case が正規形である。`docs.repo_slot`、`docs.surfaces.*.repo_slot`、`docs.authorities.*.repo_slot` を使う。旧 `repoSlot` key は `migration_required=true` で失敗する。
...
- Kanbalone アダプターと初期化ツール。既定の Kanbalone イメージは `v0.9.34`
- 同梱 Kanbalone `v0.9.34` では、現在の filter によってすべての child ticket が非表示になっている場合でも、parent ticket の階層 indicator が維持される。resolved の child だけが残る場合も対象である。ticket detail / summary API は `hasChildren` を返すため、階層表示は filtered board list ではなく保存済み relation を使える。
- エージェント HTTP ワーカー境界。取得済みジョブの heartbeat を含む
- エージェントが具体化するワークスペース方式
- agent-materialized workspace materialization は live/source repository の checkout を変更してはならない。A2O は implementation 中に A2O 管理 ref / worktree を作成・更新でき、final delivery では設定された live/source repository へ merge できるが、workspace preparation 中に live repository の `HEAD` または current branch が変化した場合は失敗する。
- TypeScript、Go、Python、複数リポジトリタスクテンプレートの参照用プロダクトパッケージ
- GHCR ランタイムイメージタグ: `latest`、`0.5.85`、`sha-*`
- タグリリースでは `latest` も同時に公開する。そのため、公開完了後はリリース版タグと `latest` が同じランタイムイメージを指す前提で確認する。
- ローカルリリース判定: `tools/dev/test-core.sh` または同等の Ruby / Go / Kanban adapter 全体確認、release package doctor、local RC host smoke、および runtime 実行 / worker launcher / scheduler / Kanban / env generation 変更時の real-task local RC smoke
...
- 内部保守性: Go host launcher の agent target / install tests を残存する大きな `main_test.go` から分離し、Ruby runtime の artifact / log cleanup handlers を root CLI module から分離した。
...
- runtime zombie-process 修正に project package migration は不要である。ただし、新しい Compose の `init: true` を有効にするには upgrade 後に `a2o-runtime` container を再作成する必要がある。Docker init は container 作成時の設定であるため、単なる `docker restart` / `docker compose restart` では不十分である。可能な場合は `a2o upgrade apply 0.5.85` を使うこと。0.5.85 の upgrade finalization は、runtime 再起動前に release 管理の Compose file 参照を新しく導入した shared asset へ更新するため、以前の runtime instance が古い A2O shared Compose file を指していても `init: true` が適用される。
- 内包 Kanbalone の外部連携資格情報に project package migration は不要である。A2O を起動する host 環境に `KANBALONE_REMOTE_CREDENTIALS` または `GITHUB_TOKEN` を設定し、内包 Kanbalone service を再作成する。A2O 管理の Compose file を直接編集しないこと。配布 Compose file がこれらの環境変数を Kanbalone へ passthrough する。
- 0.5.85 の scheduler 修正に project package migration は不要である。ただし Merging lane の復旧と merge conflict の blocked 化を新 runtime で動かすため、host launcher / shared assets と runtime image の両方を更新し、runtime container を再起動して scheduler を resume すること。
- 0.5.85 の Kanbalone bootstrap 修正に project package migration は不要である。upgrade 後に host launcher / shared assets を更新したうえで `a2o kanban up` を実行し、`trigger:auto-implement`、`trigger:auto-parent`、`trigger:investigate`、draft/decomposed labels、`blocked` などの A2O 標準 coordination tag を作成すること。bootstrap は Kanbalone 既定の `done` lane が存在しても正規の `Done` lane を作成し、reorder 時には既存の追加 lane も保持する。
- runtime image には Python Kanban CLI と Python runtime は含まれない。同梱 Go の `a2o-host kanban ...` 経路と公開 `a2o kanban ...` command を使うこと。削除済み A3 package surface を runtime image 内で呼び出す互換呼び出しは `migration_required=true` で失敗するため、upgrade 前に自動化を文書化済みの `a2o` command へ置き換える。
- 0.5.85 の agent package publication 修正に project package migration は不要である。0.5.84 の upgrade で `agent package bundle checksum mismatch` により停止している operator は `a2o upgrade apply 0.5.85` を実行すること。release runtime image は `package-publication.json` が GitHub Release asset を指す場合、ローカル生成の agent package bundle を同梱しないため、host install は descriptor が指す release asset を取得して検証する。
- `a2o upgrade apply <version>` を、単一 runtime instance 向けの正規ワンステップアップグレードコマンドとして利用できる。現在の launcher を置き換える前に finalizer へ handoff し、対象 Docker image に host launcher / shared asset の導入を委譲し、`.work/a2o/runtime-instance.json` を更新し、runtime image を再起動し、`.work/a2o/agent/bin/a2o-agent` を更新して `a2o doctor` を実行する。事前に `a2o upgrade check` で適用内容を確認し、`a2o runtime pause` で scheduler を止めること。ホストを変更せず finalizer script を確認したい場合は `a2o upgrade apply <version> --dry-run` を使う。`.work/a2o/project-registry.json` を使う workspace は、registry-wide upgrade support が追加されるまで手動更新を使う。
- scheduler resilience fixes を利用するには host launcher/shared assets と runtime image の両方を 0.5.85 に更新する。`agent package verify` の transient-network warning behavior は host launcher 側、Kanbalone 503 retry と invalid-ticket isolation は runtime image 側に含まれる。
  - `docker run --rm -v "$PWD/.work/a2o:/out" ghcr.io/wamukat/a2o-engine:0.5.85 a2o host install --output-dir /out/bin --share-dir /out/share`
  - project の runtime image 参照を `ghcr.io/wamukat/a2o-engine:0.5.85` に更新する
  - runtime container を再起動し、scheduler を resume する
...
- custom worker は review disposition の scope 欄として `review_disposition.slot_scopes` を返す必要がある。0.5.85 の worker result では `review_disposition.repo_scope` を受け付けない。`"repo_alpha"` のような値は `slot_scopes: ["repo_alpha"]` に、複数リポジトリの指摘は対象 slot 名の配列に移行する。保存済み worker result は `a2o worker validate-result --request request.json --result result.json --review-slot-scope <slot>` で検証する。
...
- `A3_SHARE_DIR` は host package fallback として受け付けない。`A2O_SHARE_DIR` または `--share-dir` を使うこと。削除済みの `A3_SHARE_DIR` だけが指定された場合、host install は launcher をコピーする前に失敗する。
...
- decomposition command 実行は host launcher と同梱 host agent に依存する。`a2o runtime decomposition investigate`、`propose`、`review` を使う前に、host launcher / shared assets と runtime image を同じ版へ更新すること。古い host launcher のままでは decomposition の host-agent 実行経路を認識できない場合がある。典型的な更新手順は次の通り。
  - release image から新しい launcher を導入する: `docker run --rm -v "$PWD/.work/a2o:/out" ghcr.io/wamukat/a2o-engine:0.5.85 a2o host install --output-dir /out/bin --share-dir /out/share`
  - project の runtime image 参照を `ghcr.io/wamukat/a2o-engine:0.5.85` に更新する
  - decomposition command を実行する前に、新しい image で runtime container を再起動する
- completion hook diagnostics と observer hook を使うには、0.5.85 runtime image と同じ版の host launcher / shared assets が必要である。completion hook command は host-agent job environment で実行され、`{{a2o_root_dir}}` は agent が workspace request を受け取る前に host 側の A2O root へ展開され、その後 `{{slot}}` や `{{slot_path}}` のような slot ごとの placeholder は host agent 内で展開される。これにより、`{{a2o_root_dir}}/project-package/...` が literal のまま shell に渡ることを防ぎ、host の PATH 上にある `task` などの tool を呼び出せる。`docker run --rm -v "$PWD/.work/a2o:/out" ghcr.io/wamukat/a2o-engine:0.5.85 a2o host install --output-dir /out/bin --share-dir /out/share` で新しい host launcher / shared assets を導入し、project の runtime image 参照を `ghcr.io/wamukat/a2o-engine:0.5.85` に更新する。その後、`a2o agent install --target auto --output ./.work/a2o/agent/bin/a2o-agent` を実行し、`a2o doctor` で hook が使う required command が利用可能であることを確認してから、`a2o runtime resume --interval 60s --agent-poll-interval 5s` で再開する。
- 0.5.85 の decomposition draft acceptance は、既定で generated parent automation を有効化する。upgrade 後の `a2o runtime decomposition accept-drafts` は、`--no-parent-auto` を渡さない限り generated parent に `trigger:auto-parent` と accepted child の `repo:*` label の和集合を付ける。この挙動を使う前に host launcher / shared assets と runtime image の両方を更新すること。
...
- docs surface の repository slot key を snake_case へ移行すること。`docs.repoSlot` は `docs.repo_slot` に、`docs.surfaces.*.repoSlot` は `docs.surfaces.*.repo_slot` に、`docs.authorities.*.repoSlot` は `docs.authorities.*.repo_slot` に変更する。移行後に `a2o project validate` を実行し、`migration_required=true` が残っていないことを確認する。
- `docs.surfaces` を採用する project package は、各 surface の `repo_slot` が repo source として設定されていること、および cross-repo `docs.authorities.*.repo_slot` が source-of-truth file を持つ repo を指していることを確認する。
...
- `publish.commit_hook_policy` は公開リリース前に置き換えられたため受け付けない。代わりに `publish.commit_preflight.native_git_hooks` を使う。古い key を使った場合は、publish commit hook の挙動を黙って変えず、project package または agent request validation で明示的に失敗する。
