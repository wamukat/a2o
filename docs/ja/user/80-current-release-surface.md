...
- decomposition source ticket が外部 issue から import されたものの場合、A2O は正規化した remote metadata を generated parent と child-creation evidence の `source_remote` に残す。draft child は local planning ticket のままであり、remote metadata は直接コピーしない。
