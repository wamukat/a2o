...
| `scripts/a3-projects/portal/portal_scheduler_launcher.rb` | Portal root glue | Deleted | The run-once path no longer launches through this wrapper, and reconcile observes `operator-tests/runtime_agent_run_once.sh` instead of the retired local scheduler process. |
