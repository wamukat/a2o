  ...
  module Domain
    class RunnableTaskAssessment
      ...
      def self.evaluate(task:, tasks:)
        ...
        return new(task: task, phase: phase, reason: :decomposition_requested) if task.decomposition_requested?
        return new(task: task, phase: phase, reason: :draft_child_not_accepted) if task.draft_child? && !task.auto_implement_requested?
