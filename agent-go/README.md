...
```sh
VERSION=0.5.16 ./scripts/build-release.sh
```
...
- `dist/a2o-agent-packages-0.5.16.tar.gz`
...
```sh
./scripts/install-release.sh dist/a3-agent-0.5.16-linux-amd64.tar.gz
```
...
docker run --rm \
  ...
  ghcr.io/wamukat/a2o-engine:0.5.16 \
  a2o host install \
    --output-dir /install/bin \
    --share-dir /install/share/a2o \
    --runtime-image ghcr.io/wamukat/a2o-engine:0.5.16
```
