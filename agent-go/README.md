...
```sh
VERSION=0.5.30 ./scripts/build-release.sh
```
...
- archives, for example `dist/a3-agent-0.5.30-linux-amd64.tar.gz`
- `dist/checksums.txt`
- `dist/release-manifest.jsonl`
- `dist/package-compatibility.json`
- `dist/a2o-agent-packages-0.5.30.tar.gz`
...
```sh
./scripts/install-release.sh dist/a3-agent-0.5.30-linux-amd64.tar.gz
```
...
docker run --rm \
  ...
  ghcr.io/wamukat/a2o-engine:0.5.30 \
  a2o host install \
    --output-dir /install/bin \
    --share-dir /install/share/a2o \
    --runtime-image ghcr.io/wamukat/a2o-engine:0.5.30
```
