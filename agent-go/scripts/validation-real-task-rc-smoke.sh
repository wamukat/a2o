...
VERSION="${VERSION:-}"
if [[ -z "${VERSION}" ]]; then
  echo "VERSION is required, for example: VERSION=0.5.72 $0" >&2
  exit 2
fi
...
grep -Fq '"stage":"completion_hooks_start"' "${WORKSPACE_DIR}/.work/a2o/runtime-host-agent/agent.log"
grep -Fq '"stage":"completion_hooks_done"' "${WORKSPACE_DIR}/.work/a2o/runtime-host-agent/agent.log"
