...
RSpec.describe A3RerunQuarantine do
  ...
  it "moves default rerun paths outside the issue workspace" do
    Dir.mktmpdir("a3-rerun-quarantine-") do |dir|
      root = Pathname(dir)
      issue_workspace = root.join(".work", "a3", "issues", "portal", "portal-2700")
      repo = issue_workspace.join("member-portal-ui-app")
      ...
      expect(quarantine_root.join("member-portal-ui-app", "target")).to exist
      expect(quarantine_root.join("member-portal-ui-app", ".work")).to exist
  ...
  it "moves broken top-level support bridges" do
    Dir.mktmpdir("a3-rerun-quarantine-") do |dir|
      ...
      broken_bridge = issue_workspace.join("member-portal-starters")
      File.symlink(".support/member-portal-ui-app/member-portal-starters", broken_bridge)
      ...
      expect(quarantine_root.join("member-portal-starters")).to be_symlink
      expect(File.readlink(quarantine_root.join("member-portal-starters").to_s)).to eq(".support/member-portal-ui-app/member-portal-starters")
    end
  end
