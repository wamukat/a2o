      ...
      expect(runtime_environment.operator_summary).to eq(
        {
          ...
          "observability_boundary_model" => "operator_logs_root=#{File.join(dir, 'logs')} blocked_diagnosis_root=#{File.join(dir, 'blocked_diagnoses')} evidence_root=#{File.join(dir, 'evidence')} canary_output=stdout_only workspace_debug_reference=path_only",
          ...
          "runtime_contract" => "manifest_schema_version=1 required_manifest_schema_version=1 required_preset_schema_version=1 preset_schema_versions=base:1 repo_source_strategy=none repo_source_slots= secret_delivery_mode=environment_variable secret_reference=A3_SECRET scheduler_store_migration_state=not_required",
          "schema_action" => "update runtime package manifest schema to 1",
          ...
          "runtime_canary_command" => "bin/a3 doctor-runtime #{manifest_path} --preset-dir #{preset_dir} --storage-backend json --storage-dir #{dir} && bin/a3 execute-until-idle #{manifest_path} --preset-dir #{preset_dir} --storage-backend json --storage-dir #{dir}",
      ...
      expect(runtime_environment.operator_summary).to eq(
        {
          ...
          "upgrade_contract" => "image_upgrade=independent manifest_schema_version=1 preset_schema_version=1 state_migration=explicit",
          "fail_fast_policy" => "manifest_schema_mismatch=fail_fast preset_schema_conflict=fail_fast writable_mount_missing=fail_fast secret_missing=fail_fast scheduler_store_migration_pending=fail_fast",
          "runtime_contract" => "manifest_schema_version=1 required_manifest_schema_version=1 required_preset_schema_version=1 preset_schema_versions= repo_source_strategy=none repo_source_slots= secret_delivery_mode=environment_variable secret_reference=A3_SECRET scheduler_store_migration_state=not_required",
          "schema_action" => "update runtime package manifest schema to 1",
          "repo_source_action" => "no repo source action required",
          "secret_delivery_action" => "provide secrets via environment variable A3_SECRET",
          "scheduler_store_migration_action" => "scheduler store migration not required",
