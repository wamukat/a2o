...
RSpec.describe A3::CLI do
  ...
  it "recovers a missing runtime parent from kanban when the parent is already merging" do
    Dir.mktmpdir do |dir|
      create_git_repo_source(dir, name: "repo-alpha-source", file_content: "repo_alpha source\n")
      create_git_repo_source(dir, name: "repo-beta-source", file_content: "repo_beta source\n")
      repo_sources = {
      ...
      repo_sources.each_value do |repo_path|
        head = `git -C #{Shellwords.escape(repo_path)} rev-parse HEAD`.strip
        system("git", "-C", repo_path, "update-ref", "refs/heads/a2o/parent/Sample-5200", head, exception: true, out: File::NULL, err: File::NULL)
      end
      fake_cli = create_fake_kanban_cli(
        dir,
        mutate_state_on_transition: true,
        snapshots: [
          {
            "id" => 5200,
            "ref" => "Sample#5200",
            "title" => "Parent merge recovery",
            "description" => "This parent was visible on kanban but missing from runtime storage.",
            "status" => "Merging",
            "labels" => ["repo:both", "trigger:auto-parent"],
            "parent_ref" => nil
          },
          {
            ...
            "description" => "Child already done.",
            "status" => "Done",
            "labels" => ["repo:ui-app", "trigger:auto-implement"],
      ...
      parent = task_repository.fetch("Sample#5200")
      transitions = read_fake_kanban_transitions(fake_cli.fetch(:transitions_path))

      expect(out.string).to include("executed 1 task(s); idle=true stop_reason=idle")
      expect(out.string).to include("steps=Sample#5200:merge")
      expect(parent.kind).to eq(:parent)
      expect(parent.child_refs).to eq(["Sample#5201"])
      expect(parent.external_task_id).to eq(5200)
      expect(parent.status).to eq(:done)
      expect(transitions.map { |item| item.fetch("argv").last }).to include("Done")
    end
  end
