...
RSpec.describe A3::Domain::RuntimePackageDescriptor do
  it "derives a distribution contract from the image version by default" do
    ...
    expect(descriptor.operator_summary.fetch("schema_contract")).to eq("manifest_schema_version=1 required_manifest_schema_version=1")
    ...
    expect(descriptor.operator_summary.fetch("schema_action")).to eq("update runtime package manifest schema to 1")
    ...
    expect(descriptor.operator_summary.fetch("runtime_configuration_model")).to eq("manifest_path=required preset_dir=required storage_backend=required state_root=required workspace_root=required artifact_root=required repo_source_strategy=required repository_metadata=required authoritative_branch_resolution=required integration_target_resolution=required secret_reference=required")
    ...
    expect(descriptor.operator_summary.fetch("observability_boundary_model")).to eq("operator_logs_root=/tmp/runtime/state/logs blocked_diagnosis_root=/tmp/runtime/state/blocked_diagnoses evidence_root=/tmp/runtime/state/evidence canary_output=stdout_only workspace_debug_reference=path_only")
