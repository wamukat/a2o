...
RSpec.describe A3::CLI::CommandRouter do
  ...
  describe ".session_kind_for" do
    it "exposes explicit session kinds for storage, manifest, and runtime commands" do
      ...
      expect(described_class.session_kind_for("run-decomposition-investigation")).to eq(:runtime)
      ...
      expect(described_class.session_kind_for("accept-decomposition-drafts")).to eq(:storage)
      expect(described_class.session_kind_for("show-decomposition-status")).to eq(:storage)
      expect(described_class.session_kind_for("cleanup-decomposition-trial")).to eq(:storage)
