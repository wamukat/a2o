...
RSpec.describe A3::CLI do
  ...
  it "shows task and run inspection views through sqlite backend" do
    Dir.mktmpdir do |dir|
      ...
      expect(out.string).to include("run run-1 task=A3-v2#child phase=review workspace=runtime_workspace source=detached_commit:head456 outcome=blocked")
      expect(out.string).to include("review_target=base123..head456")
      expect(out.string).to include("latest_execution phase=review summary=review launch could not resolve runtime workspace")
      ...
      expect(out.string).to include("runtime task_kind=child repo_scope=repo_alpha phase=review")
      ...
      expect(out.string).to include("latest_blocked phase=review summary=review launch could not resolve runtime workspace")
  ...
  it "shows runtime package recovery commands when manifest context is provided" do
    with_env(
      ...
      Dir.mktmpdir do |dir|
        ...
        expect(out.string).to include("runtime_package_runtime_canary_command=bin/a3 doctor-runtime #{manifest_path} --preset-dir #{preset_dir} --storage-backend sqlite --storage-dir #{dir} && bin/a3 migrate-scheduler-store #{manifest_path} --preset-dir #{preset_dir} --storage-backend sqlite --storage-dir #{dir} && bin/a3 execute-until-idle #{manifest_path} --preset-dir #{preset_dir} --storage-backend sqlite --storage-dir #{dir}")
