...
RSpec.describe A3::Application::RecoverPersistedRerun do
  subject(:use_case) do
    described_class.new(
  ...
  let(:plan_persisted_rerun) do
    A3::Application::PlanPersistedRerun.new(
  ...
  let(:doctor_result) do
    instance_double(
      ...
      execution_modes_summary: "one_shot_cli=bin/a3 doctor-runtime /tmp/manifest.yml | bin/a3 migrate-scheduler-store /tmp/manifest.yml | bin/a3 execute-until-idle /tmp/manifest.yml ; scheduler_loop=bin/a3 execute-until-idle /tmp/manifest.yml ; doctor_inspect=bin/a3 doctor-runtime /tmp/manifest.yml",
      ...
      recommended_execution_mode_command: "bin/a3 doctor-runtime /tmp/manifest.yml && bin/a3 migrate-scheduler-store /tmp/manifest.yml && bin/a3 execute-until-idle /tmp/manifest.yml",
      doctor_command_summary: "bin/a3 doctor-runtime /tmp/manifest.yml",
      runtime_command_summary: "bin/a3 execute-until-idle /tmp/manifest.yml",
  ...
  it "returns runtime package recovery guidance as part of the canonical result" do
    ...
    expect(result.recovery).to have_attributes(
      ...
      runtime_package_guidance: "startup blocked by scheduler_store_migration; apply scheduler store migration before startup; run bin/a3 doctor-runtime /tmp/manifest.yml",
      ...
      runtime_package_execution_modes: "one_shot_cli=bin/a3 doctor-runtime /tmp/manifest.yml | bin/a3 migrate-scheduler-store /tmp/manifest.yml | bin/a3 execute-until-idle /tmp/manifest.yml ; scheduler_loop=bin/a3 execute-until-idle /tmp/manifest.yml ; doctor_inspect=bin/a3 doctor-runtime /tmp/manifest.yml",
      ...
      runtime_package_recommended_execution_mode_command: "bin/a3 doctor-runtime /tmp/manifest.yml && bin/a3 migrate-scheduler-store /tmp/manifest.yml && bin/a3 execute-until-idle /tmp/manifest.yml",
      runtime_package_next_command: "bin/a3 migrate-scheduler-store /tmp/manifest.yml",
      runtime_package_doctor_command: "bin/a3 doctor-runtime /tmp/manifest.yml",
      runtime_package_migration_command: "bin/a3 migrate-scheduler-store /tmp/manifest.yml",
      runtime_package_runtime_command: "bin/a3 execute-until-idle /tmp/manifest.yml",
  ...
  context "when rerun planning requires operator action" do
    let(:rerun_plan) { Struct.new(:task, :run, :decision).new(task, run, :requires_operator_action) }
    let(:plan_persisted_rerun) { instance_double(A3::Application::PlanPersistedRerun, call: rerun_plan) }

    it "returns an operator-facing recovery result instead of raising" do
      result = use_case.call(
        task_ref: task.ref,
        run_ref: run.ref,
        runtime_package: runtime_package,
      ...
      expect(result.recovery_action).to eq(:diagnose_blocked)
      expect(result.target_phase).to eq(:review)
      expect(result.recovery.runtime_package_guidance).to include("startup blocked by scheduler_store_migration")
    end
  end
end
