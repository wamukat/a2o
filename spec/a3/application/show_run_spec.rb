...
RSpec.describe A3::Application::ShowRun do
  ...
  it "surfaces implementation-side review evidence on the latest execution snapshot" do
    ...
    result = use_case.call(run_ref: "run-impl-1", runtime_package: runtime_package)
  ...
  it "adds runtime package recovery commands when runtime package is provided" do
    ...
    doctor_result = instance_double(
      ...
      execution_modes_summary: "one_shot_cli=bin/a3 doctor-runtime /tmp/runtime/project.yaml | bin/a3 migrate-scheduler-store /tmp/runtime/project.yaml | bin/a3 execute-until-idle /tmp/runtime/project.yaml ; scheduler_loop=bin/a3 execute-until-idle /tmp/runtime/project.yaml ; doctor_inspect=bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      execution_mode_contract_summary: "one_shot_cli=operator_driven_doctor_migration_runtime ; scheduler_loop=continuous_runnable_processing_after_runtime_ready ; doctor_inspect=configuration_and_mount_validation_only",
      contract_health: "project_config_schema=ok preset_schema=ok repo_sources=missing secret_delivery=ok scheduler_store_migration=ok",
      schema_action_summary: "no schema action required",
      preset_schema_action_summary: "no preset schema action required",
      ...
      doctor_command_summary: "bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      runtime_command_summary: "bin/a3 execute-until-idle /tmp/runtime/project.yaml",
      operator_guidance: "startup blocked by repo_sources; provide writable repo sources for repo_alpha; run bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      next_command: "bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      migration_command_summary: "bin/a3 migrate-scheduler-store /tmp/runtime/project.yaml",
      runtime_validation_command_summary: "bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      startup_sequence: "doctor=bin/a3 doctor-runtime /tmp/runtime/project.yaml migrate=blocked runtime=blocked",
    ...
    expect(result.recovery).to have_attributes(
      runtime_package_guidance: "startup blocked by repo_sources; provide writable repo sources for repo_alpha; run bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      runtime_package_contract_health: "project_config_schema=ok preset_schema=ok repo_sources=missing secret_delivery=ok scheduler_store_migration=ok",
