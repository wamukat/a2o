...
RSpec.describe A3::Application::ShowBlockedDiagnosis do
  subject(:use_case) do
    described_class.new(
  ...
  let(:doctor_result) do
    instance_double(
      ...
      execution_modes_summary: "one_shot_cli=bin/a3 doctor-runtime /tmp/runtime/project.yaml | bin/a3 migrate-scheduler-store /tmp/runtime/project.yaml | bin/a3 execute-until-idle /tmp/runtime/project.yaml ; scheduler_loop=bin/a3 execute-until-idle /tmp/runtime/project.yaml ; doctor_inspect=bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      ...
      next_command: "bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      migration_command_summary: "bin/a3 migrate-scheduler-store /tmp/runtime/project.yaml",
      runtime_validation_command_summary: "bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      startup_sequence: "doctor=bin/a3 doctor-runtime /tmp/runtime/project.yaml migrate=blocked runtime=blocked",
      ...
      recommended_execution_mode_command: "bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      doctor_command_summary: "bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      runtime_command_summary: "bin/a3 execute-until-idle /tmp/runtime/project.yaml",
  ...
  let(:diagnosis) do
    A3::Domain::BlockedDiagnosis.new(
      ...
      infra_diagnostics: { "missing_path" => "/tmp/repo-beta", "stderr" => "ENOENT" }
    )
  end
  ...
  let(:run) do
    ...
    ).append_blocked_diagnosis(diagnosis)
  end
  ...
  it "returns the persisted blocked diagnosis and operator-facing evidence summary" do
    result = use_case.call(task_ref: task.ref, run_ref: run.ref, runtime_package: runtime_package)

    expect(result.task).to eq(task)
    expect(result.run).to eq(run)
    expect(result.diagnosis).to eq(diagnosis)
    expect(result.evidence_summary.workspace_kind).to eq(:runtime_workspace)
    expect(result.evidence_summary.source_type).to eq(:detached_commit)
    expect(result.evidence_summary.source_ref).to eq("head456")
    expect(result.evidence_summary.review_base).to eq("base123")
    expect(result.evidence_summary.review_head).to eq("head456")
    expect(result.evidence_summary.edit_scope).to eq([:repo_alpha])
    expect(result.evidence_summary.verification_scope).to eq([:repo_alpha, :repo_beta])
    expect(result.evidence_summary.artifact_owner_ref).to eq("A3-v2#3022")
  ...
  it "fails fast when the persisted run has no blocked diagnosis bundle" do
    run_repository.save(
      A3::Domain::Run.new(
        ref: "run-2",
        task_ref: task.ref,
    ...
    expect do
      use_case.call(task_ref: task.ref, run_ref: "run-2", runtime_package: runtime_package)
    end.to raise_error(A3::Domain::ConfigurationError, /blocked diagnosis not found/)
  end

  it "enriches blocked diagnosis recovery with runtime package commands when provided" do
    runtime_package = instance_double(A3::Domain::RuntimePackageDescriptor)
    doctor_result = instance_double(
      A3::Application::DoctorRuntimeEnvironment::Result,
      execution_modes_summary: "one_shot_cli=bin/a3 doctor-runtime /tmp/runtime/project.yaml | bin/a3 migrate-scheduler-store /tmp/runtime/project.yaml | bin/a3 execute-until-idle /tmp/runtime/project.yaml ; scheduler_loop=bin/a3 execute-until-idle /tmp/runtime/project.yaml ; doctor_inspect=bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      execution_mode_contract_summary: "one_shot_cli=operator_driven_doctor_migration_runtime ; scheduler_loop=continuous_runnable_processing_after_runtime_ready ; doctor_inspect=configuration_and_mount_validation_only",
      contract_health: "project_config_schema=ok preset_schema=ok repo_sources=ok secret_delivery=missing scheduler_store_migration=ok",
      schema_action_summary: "no schema action required",
      preset_schema_action_summary: "no preset schema action required",
      repo_source_action_summary: "no repo source action required",
      secret_delivery_action_summary: "provide secrets via environment variable A3_SECRET",
      ...
      doctor_command_summary: "bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      runtime_command_summary: "bin/a3 execute-until-idle /tmp/runtime/project.yaml",
      operator_guidance: "startup blocked by secret_delivery; provide secrets via environment variable A3_SECRET; run bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      next_command: "bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      migration_command_summary: "bin/a3 migrate-scheduler-store /tmp/runtime/project.yaml",
      runtime_validation_command_summary: "bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      startup_sequence: "doctor=bin/a3 doctor-runtime /tmp/runtime/project.yaml migrate=blocked runtime=blocked",
      startup_blockers: "secret_delivery",
      distribution_summary: {
        "persistent_state_model" => "scheduler_state_root=/tmp/runtime/state/scheduler task_repository_root=/tmp/runtime/state/tasks run_repository_root=/tmp/runtime/state/runs evidence_root=/tmp/runtime/state/evidence blocked_diagnosis_root=/tmp/runtime/state/blocked_diagnoses artifact_owner_cache_root=/tmp/runtime/state/artifact_owner_cache logs_root=/tmp/runtime/state/logs workspace_root=/tmp/runtime/state/workspaces artifact_root=/tmp/runtime/state/artifacts",
        "retention_policy" => "terminal_workspace_cleanup=retention_policy_controlled blocked_evidence_retention=independent_from_scheduler_cleanup image_upgrade_cleanup_trigger=none",
        "materialization_model" => "repo_slot_namespace=task_workspace_fixed implementation_workspace=ticket_workspace review_workspace=runtime_workspace verification_workspace=runtime_workspace merge_workspace=runtime_workspace runtime_workspace_kind=logical_phase_workspace physical_workspace_layout=worker_gateway_mode_defined agent_materialized_runtime_workspace=per_run_materialized missing_repo_rescue=forbidden source_descriptor_alignment=required_before_phase_start",
    ...
    expect(result.recovery).to have_attributes(
      runtime_package_guidance: "startup blocked by secret_delivery; provide secrets via environment variable A3_SECRET; run bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      runtime_package_contract_health: "project_config_schema=ok preset_schema=ok repo_sources=ok secret_delivery=missing scheduler_store_migration=ok",
      runtime_package_execution_modes: "one_shot_cli=bin/a3 doctor-runtime /tmp/runtime/project.yaml | bin/a3 migrate-scheduler-store /tmp/runtime/project.yaml | bin/a3 execute-until-idle /tmp/runtime/project.yaml ; scheduler_loop=bin/a3 execute-until-idle /tmp/runtime/project.yaml ; doctor_inspect=bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      ...
      runtime_package_recommended_execution_mode_command: "bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      ...
      runtime_package_next_execution_mode_command: "bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      runtime_package_next_command: "bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      runtime_package_doctor_command: "bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      runtime_package_migration_command: "bin/a3 migrate-scheduler-store /tmp/runtime/project.yaml",
      runtime_package_runtime_command: "bin/a3 execute-until-idle /tmp/runtime/project.yaml",
      runtime_package_runtime_validation_command: "bin/a3 doctor-runtime /tmp/runtime/project.yaml",
      runtime_package_startup_sequence: "doctor=bin/a3 doctor-runtime /tmp/runtime/project.yaml migrate=blocked runtime=blocked",
      runtime_package_startup_blockers: "secret_delivery",
      ...
      runtime_package_upgrade_contract: "image_upgrade=independent project_config_schema_version=1 preset_schema_version=1 state_migration=explicit",
      runtime_package_fail_fast_policy: "project_config_schema_mismatch=fail_fast preset_schema_conflict=fail_fast writable_mount_missing=fail_fast secret_missing=fail_fast scheduler_store_migration_pending=fail_fast"
    )
  end

  it "does not attach worker response bundle from a later non-blocked phase" do
    later_phase_run = run.append_phase_evidence(
      phase: :verification,
      source_descriptor: source_descriptor,
      scope_snapshot: scope_snapshot,
    ...
    expect(result.worker_response_bundle).to be_nil
    expect(result.diagnosis.infra_diagnostics).not_to include("worker_response_bundle")
  end
end
