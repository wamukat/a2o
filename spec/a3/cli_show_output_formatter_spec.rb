...
RSpec.describe A3::CLI::ShowOutputFormatter do
  def build_recovery_view(decision)
    ...
    when :requires_operator_action
      A3::Domain::OperatorInspectionReadModel::RunView::RecoveryView.new(
        ...
        runtime_package_operator_action_command: "bin/a3 doctor-runtime /tmp/runtime/project.yaml",
        ...
        runtime_package_runtime_validation_command: "bin/a3 doctor-runtime /tmp/runtime/project.yaml && bin/a3 execute-until-idle /tmp/runtime/project.yaml",
        ...
        runtime_package_startup_sequence: "doctor=bin/a3 doctor-runtime /tmp/runtime/project.yaml migrate=skip runtime=bin/a3 execute-until-idle /tmp/runtime/project.yaml",
  ...
  it "formats blocked diagnosis lines with the raw worker response bundle when present" do
    ...
    recovery = instance_double(
      ...
      runtime_package_guidance: "run doctor-runtime and inspect repo sources, secret delivery, and scheduler store migration before rerun",
      runtime_package_contract_health: "project_config_schema=ok preset_schema=ok repo_sources=missing secret_delivery=missing scheduler_store_migration=ok",
      runtime_package_execution_modes: "one_shot_cli=bin/a3 doctor-runtime /tmp/runtime/project.yaml ; scheduler_loop=bin/a3 execute-until-idle /tmp/runtime/project.yaml ; doctor_inspect=bin/a3 doctor-runtime /tmp/runtime/project.yaml",
    ...
    expect(result).to include("runtime_package_execution_modes=one_shot_cli=bin/a3 doctor-runtime /tmp/runtime/project.yaml ; scheduler_loop=bin/a3 execute-until-idle /tmp/runtime/project.yaml ; doctor_inspect=bin/a3 doctor-runtime /tmp/runtime/project.yaml")
    ...
    expect(result).to include("runtime_package_recommended_execution_mode_command=bin/a3 doctor-runtime /tmp/runtime/project.yaml")
    ...
    expect(result).to include("runtime_package_next_execution_mode_command=bin/a3 doctor-runtime /tmp/runtime/project.yaml")
    expect(result).to include("runtime_package_next_command=bin/a3 doctor-runtime /tmp/runtime/project.yaml")
    expect(result).to include("runtime_package_migration_command=bin/a3 migrate-scheduler-store /tmp/runtime/project.yaml")
    expect(result).to include("runtime_package_runtime_validation_command=bin/a3 doctor-runtime /tmp/runtime/project.yaml && bin/a3 execute-until-idle /tmp/runtime/project.yaml")
    expect(result).to include("runtime_package_startup_sequence=doctor=bin/a3 doctor-runtime /tmp/runtime/project.yaml migrate=skip runtime=bin/a3 execute-until-idle /tmp/runtime/project.yaml")
  ...
  it "formats task lines through the task formatter" do
    ...
    expect(result).to include("skill_feedback_targets=project_skill:1")
    expect(result).to include("skill_feedback_categories=missing_context:1")
    expect(result).to include("skill_feedback_states=new:1")
  ...
  it "formats run lines through the run formatter" do
    ...
    expect(result).to include("skill_feedback_count=1")
    expect(result).to include("skill_feedback_pending_review=1 action=review_or_convert_to_ticket")
    expect(result).to include("skill_feedback category=review_gap target=project_skill state=new repo_scope=repo_alpha skill_path=skills/review/default.md confidence=high")
  ...
  it "formats merge recovery evidence in run lines" do
    run_view = A3::Domain::OperatorInspectionReadModel::RunView.new(
      ...
      source_ref: "refs/heads/a2o/parent/Sample-201",
      ...
      evidence_summary: A3::Domain::OperatorInspectionReadModel::EvidenceSummary.new(
        ...
        artifact_snapshot_version: "refs/heads/a2o/parent/Sample-201",
      ...
      latest_execution: A3::Domain::OperatorInspectionReadModel::RunView::ExecutionSnapshot.new(
        ...
        diagnostics: {
          "merge_recovery" => {
            "status" => "recovered",
            "target_ref" => "refs/heads/a2o/parent/Sample-201",
    ...
    expect(result).to include("merge_recovery status=recovered target_ref=refs/heads/a2o/parent/Sample-201 source_ref=refs/heads/a2o/work/Sample-245")
  ...
  it "formats implementation-side review evidence in run lines" do
    run_view = A3::Domain::OperatorInspectionReadModel::RunView.new(
      ...
      source_type: :branch_head,
      source_ref: "refs/heads/a2o/work/child",
      terminal_outcome: :completed,
      evidence_summary: A3::Domain::OperatorInspectionReadModel::EvidenceSummary.new(
        workspace_kind: :ticket_workspace,
        source_type: :branch_head,
  ...
  it "omits finding_key in run output when clean review evidence has no finding key" do
    run_view = A3::Domain::OperatorInspectionReadModel::RunView.new(
      ref: "run-123",
      task_ref: "A3-v2#child",
      task_kind: :child,
    ...
    expect(result).to include("review_disposition kind=completed slot_scopes=repo_alpha")
    expect(result).not_to include(a_string_matching(/finding_key=/))
  end
